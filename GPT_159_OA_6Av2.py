...
# Layer windows. These are canonical Qwen-1.5B style windows.
# The script will automatically intersect them with actually available layers.
...
PRED_COLUMNS = [
    "k",
    "shallow_task_f1", "mid_task_f1", "crit_task_f1", "late_task_f1",
    "shallow_pc1", "mid_pc1", "crit_pc1", "late_pc1",
    "shallow_dom", "mid_dom", "crit_dom", "late_dom",
    "shallow_erank", "mid_erank", "crit_erank", "late_erank",
    "pc1_gain_shallow_to_crit", "pc1_gain_shallow_to_late",
    "dom_gain_shallow_to_crit", "dom_gain_shallow_to_late",
    "erank_drop_shallow_to_late",
]

def available_window(all_layers, canonical_layers):
    all_layers = set(map(int, all_layers))
    return [l for l in canonical_layers if l in all_layers]
...
def shallow_to_late_prediction(layer_df, direction_df):
    ...
    if layer_df is None or layer_df.empty or "k" not in layer_df.columns:
        return pd.DataFrame(columns=PRED_COLUMNS)

    for k in sorted(layer_df["k"].unique()):
        ldf = layer_df[layer_df["k"] == k].copy()
        actual_layers = sorted(ldf["layer_index"].unique().tolist())

        shallow_layers = available_window(actual_layers, SHALLOW_LAYERS)
        mid_layers = available_window(actual_layers, MID_LAYERS)
        crit_layers = available_window(actual_layers, CRIT_LAYERS)
        late_layers = available_window(actual_layers, LATE_LAYERS)

        # Fallback for smaller / shifted models:
        # split actual layers into approximate quartiles if canonical windows are missing.
        if not shallow_layers or not mid_layers or not crit_layers or not late_layers:
            n = len(actual_layers)
            if n >= 8:
                q1 = max(1, int(round(n * 0.25)))
                q2 = max(q1 + 1, int(round(n * 0.70)))
                q3 = max(q2 + 1, int(round(n * 0.82)))

                shallow_layers = actual_layers[:q1]
                mid_layers = actual_layers[q1:q2]
                crit_layers = actual_layers[q2:q3]
                late_layers = actual_layers[q3:]
            else:
                # Not enough layers for this analysis.
                rows.append({c: np.nan for c in PRED_COLUMNS} | {"k": int(k)})
                continue
        ...
        row["pc1_gain_shallow_to_crit"] = row["crit_pc1"] - row["shallow_pc1"]
        row["pc1_gain_shallow_to_late"] = row["late_pc1"] - row["shallow_pc1"]
        row["dom_gain_shallow_to_crit"] = row["crit_dom"] - row["shallow_dom"]
        row["dom_gain_shallow_to_late"] = row["late_dom"] - row["shallow_dom"]
        row["erank_drop_shallow_to_late"] = row["shallow_erank"] - row["late_erank"]
...
def verdict_summary(layer_df, pred_df, tau_df):
    ...
    if layer_df is None or layer_df.empty or "k" not in layer_df.columns:
        return pd.DataFrame([{
            "k": np.nan,
            "score": 0,
            "verdict": "FAIL: layer_df empty or missing k",
        }])

    if pred_df is None or pred_df.empty or "k" not in pred_df.columns:
        return pd.DataFrame([{
            "k": int(k),
            "task_separable": False,
            "pc1_growth": False,
            "dominance_growth": False,
            "erank_collapse": False,
            "tau_support": False,
            "score": 0,
            "verdict": "FAIL/Mixed: prediction summary empty; check available layer windows",
        } for k in sorted(layer_df["k"].unique())])

    if tau_df is None or tau_df.empty or "k" not in tau_df.columns:
        tau_df = pd.DataFrame(columns=["k", "in_L20_L22"])

    for k in sorted(layer_df["k"].unique()):
        pdf = pred_df[pred_df["k"] == k]
        tdf = tau_df[tau_df["k"] == k] if "k" in tau_df.columns else pd.DataFrame()
        ...
        if pdf.empty:
            rows.append({
                ...
                "tau_support": False,
                "score": 0,
                "verdict": "FAIL/Mixed: no prediction row for this k",
            })
            continue

        p = pdf.iloc[0]

        def finite_gt(name, threshold):
            return name in p.index and np.isfinite(p[name]) and p[name] > threshold

        task_separable = finite_gt("shallow_task_f1", 0.45) or finite_gt("mid_task_f1", 0.50)
        pc1_growth = finite_gt("pc1_gain_shallow_to_late", 0.03)
        dominance_growth = finite_gt("dom_gain_shallow_to_late", 0.10)
        erank_collapse = finite_gt("erank_drop_shallow_to_late", 0.05)
        tau_support = bool(tdf["in_L20_L22"].mean() >= 0.25) if len(tdf) and "in_L20_L22" in tdf.columns else False
        ...
        rows.append({
            ...
            "shallow_task_f1": p.get("shallow_task_f1", np.nan),
            "mid_task_f1": p.get("mid_task_f1", np.nan),
            "crit_task_f1": p.get("crit_task_f1", np.nan),
            "late_task_f1": p.get("late_task_f1", np.nan),

            "shallow_pc1": p.get("shallow_pc1", np.nan),
            "crit_pc1": p.get("crit_pc1", np.nan),
            "late_pc1": p.get("late_pc1", np.nan),

            "pc1_gain_shallow_to_late": p.get("pc1_gain_shallow_to_late", np.nan),
            "dom_gain_shallow_to_late": p.get("dom_gain_shallow_to_late", np.nan),
            "erank_drop_shallow_to_late": p.get("erank_drop_shallow_to_late", np.nan),

            "tau_L20_L22_frac": float(tdf["in_L20_L22"].mean()) if len(tdf) and "in_L20_L22" in tdf.columns else np.nan,
        })
