# ============================================================
# UA-1B: True Competing Basin Audit
#
# Motivation:
#   UA-1M found strong evidence that L7-L19 mid-layer features
#   predict L20-L22 critical gap and L23-L26 basin gap.
#
#   However, "ambiguous_branch" behaved more like clean-noise than
#   true basin competition:
#
#       mid_mean > 0
#       crit_mean > 0
#       basin_mean > 0
#
#   UA-1B therefore constructs stronger 50/50 competition conditions.
#
# Core hypothesis:
#
#   There should exist three trajectory families:
#
#       Positive trajectory:
#           clean basin C dominates.
#
#       Near-zero / critical trajectory:
#           true competing basins remain close through L7-L19,
#           then may split around L20-L22.
#
#       Negative trajectory:
...
# Interpretation:
#
#       Gap_l > 0: C / clean basin dominates.
#       Gap_l ~ 0: basin competition / critical balance.
#       Gap_l < 0: E / conflict basin dominates.
#
# Main outputs:
#   ua1b_dataset.csv
#   ua1b_gap_trajectory.csv
#   ua1b_feature_table.csv
#   ua1b_condition_feature_profile.csv
#   ua1b_prediction_summary.json
#   ua1b_summary.json
#
# ============================================================
...
SAVE_DIR = Path("./ua1b_true_competing_basin_outputs")
...
# Condition design:
#   stable_positive: clear C support.
#   weak_positive: C support + weak E distractor.
#   true_compete_balanced: exactly symmetric C/E support.
#   true_compete_order_C_first: symmetric but C appears first.
#   true_compete_order_E_first: symmetric but E appears first.
#   direct_negative: explicit E support.
#   update_negative: old C, new E.
#   exception_negative: general C, exception E.
#
# The three true_compete variants are the main audit object.
CONDITIONS = [
    {
        "condition": "stable_positive",
        "phase_target": "positive",
        "mechanism": "stable",
        "epsilon": 0.0,
        "expected_sign": +1,
    },
    {
        "condition": "weak_positive",
        "phase_target": "positive",
    ...
    {
        "condition": "true_compete_balanced",
        "phase_target": "critical",
        "mechanism": "true_competition",
        "epsilon": 2.0,
        "expected_sign": 0,
    },
    {
        "condition": "true_compete_order_C_first",
        "phase_target": "critical",
        "mechanism": "true_competition",
        "epsilon": 2.1,
        "expected_sign": 0,
    ...
    {
        "condition": "true_compete_order_E_first",
        "phase_target": "critical",
        "mechanism": "true_competition",
        "epsilon": 2.2,
        "expected_sign": 0,
    ...
    {
        "condition": "direct_negative",
        "phase_target": "negative",
        "mechanism": "competition",
        "epsilon": 3.0,
        "expected_sign": -1,
    },
    {
        "condition": "update_negative",
        "phase_target": "negative",
        "mechanism": "closure",
        "epsilon": 4.0,
        "expected_sign": -1,
...
def make_prompt(A, B, C, E, condition):
    ...
    if condition == "stable_positive":
        ...
        lines = [
            ...
            f"Fact 1: {A} belongs to {B}.",
            f"Fact 2: {B} is associated with {C}.",
            f"Unrelated note: another object sometimes uses label {E}.",
            f"Question: Which label is {A} associated with?",
            f"Answer with exactly one word: {options}.",
            "Answer:",
        ]
        ...
        lines = [
            ...
            f"Possible labels: {options}.",
            f"Fact 1: {A} belongs to {B}.",
            f"Evidence for first label: {B} is associated with {C}.",
            f"Evidence for second label: {B} is associated with {E}.",
            "Both pieces of evidence are equally current and equally reliable.",
            "There is no rule saying which one has priority.",
            f"Question: Which label is {A} associated with?",
            f"Answer with exactly one word: {options}.",
            "Answer:",
        ]
        ...
        lines = [
            ...
            f"Possible labels: {options}.",
            f"Fact 1: {A} belongs to {B}.",
            f"Record A: {B} is associated with {C}.",
            f"Record B: {B} is associated with {E}.",
            "Record A and Record B have identical authority.",
            "Do not prefer earlier or later records.",
            f"Question: Which label is {A} associated with?",
            f"Answer with exactly one word: {options}.",
            "Answer:",
        ]
        ...
        lines = [
            ...
            f"Possible labels: {options}.",
            f"Fact 1: {A} belongs to {B}.",
            f"Record A: {B} is associated with {E}.",
            f"Record B: {B} is associated with {C}.",
            "Record A and Record B have identical authority.",
            "Do not prefer earlier or later records.",
            f"Question: Which label is {A} associated with?",
            f"Answer with exactly one word: {options}.",
        ...
        lines = [
            ...
            f"Possible labels: {options}.",
            f"Fact 1: {A} belongs to {B}.",
            f"Background fact: {B} is often associated with {C}.",
            f"Direct fact: {A} is associated with {E}.",
            "Use the direct fact about the queried item.",
            f"Question: Which label is {A} associated with?",
            f"Answer with exactly one word: {options}.",
            "Answer:",
        ]
...
def build_dataset():
    ...
    for gid in range(N_GRAPHS):
        A, B = ENTITY_POOL[gid % len(ENTITY_POOL)]
        C, E = label_pairs[gid % len(label_pairs)]
        ...
        for cmeta in CONDITIONS:
            ...
            rows.append({
                ...
                "phase_target": cmeta["phase_target"],
...
for idx, row in df.iterrows():
    ...
    mid_abs_mean = float(np.mean(np.abs(gaps_mid)))
    ...
    # Criticality score: high if mid-layer gap stays close to zero.
    # Smaller mid_abs_mean means stronger critical/competition behavior.
    mid_criticality = -mid_abs_mean

    expected_sign = int(row["expected_sign"])

    if expected_sign == 0:
        # For true competition we want low absolute gap.
        directional_consistency = float(np.mean(np.abs(gaps_mid) < 0.5))
    else:
    ...
    # First layer where absolute gap exceeds a fixed threshold.
    # This is more interpretable than adaptive threshold for true competition.
    fixed_threshold = 1.0
    ...
    for l in TRACK_LAYERS:
        ...
        if first_abs_cross is None and abs(g) > fixed_threshold:
            first_abs_cross = l

        if expected_sign != 0:
            if first_sign_cross is None and np.sign(g) == expected_sign and abs(g) > fixed_threshold:
                first_sign_cross = l
    ...
    feature_rows.append({
        ...
        "mid_abs_mean_7_19": mid_abs_mean,
        "mid_criticality_7_19": mid_criticality,
...
# Merge only new feature columns back to df to avoid duplicated metadata columns.
...
feat_new_cols = [c for c in feat.columns if c not in meta_cols_already_in_df]
...
for i, row in df.iterrows():
    for l in TRACK_LAYERS:
        traj_rows.append({
            ...
            "abs_gap": abs(float(row[f"gap_L{l}"])),
...
condition_profile = (
    traj.groupby(["condition", "mechanism", "phase_target", "epsilon", "layer"])[
        ["gap", "abs_gap", "rank_gap_EminusC"]
    ]
    .agg(["mean", "std", "median"])
    .reset_index()
)
...
condition_feature_profile = (
    feat.groupby(["condition", "mechanism", "phase_target", "epsilon"])[[
        "mid_slope_7_19",
        "mid_area_7_19",
        "mid_mean_7_19",
        "mid_abs_mean_7_19",
        "mid_criticality_7_19",
        "precrit_final_19",
        "crit_mean_20_22",
        "crit_abs_mean_20_22",
        "basin_mean_23_26",
        "basin_abs_mean_23_26",
        "commit_gap_27",
        "crit_jump_from_pre",
...
prediction_summary = {
    ...
    "phase_target_classification": {
        "mid_only": group_multiclass_eval(MID_FEATURES, "phase_target", "graph_id"),
        "mid_plus_critical": group_multiclass_eval(MID_PLUS_CRIT_FEATURES, "phase_target", "graph_id"),
        "all_with_basin_commit": group_multiclass_eval(ALL_FEATURES, "phase_target", "graph_id"),
    },
    ...
    "regression_critical_abs_mean": {
        "mid_only": group_regression_eval(MID_FEATURES, "crit_abs_mean_20_22", "graph_id"),
    },
    "regression_basin_mean": {
        "mid_only": group_regression_eval(MID_FEATURES, "basin_mean_23_26", "graph_id"),
    },
    "regression_basin_abs_mean": {
        "mid_only": group_regression_eval(MID_FEATURES, "basin_abs_mean_23_26", "graph_id"),
    },
}
...
# 1. Are true competition conditions closer to zero than positive/negative conditions?
phase_profile = (
    feat.groupby("phase_target")[[
        "mid_abs_mean_7_19",
        "crit_abs_mean_20_22",
        "basin_abs_mean_23_26",
        "mid_mean_7_19",
...
# 2. First crossing distribution
cross_valid = feat[feat["first_abs_cross_layer"] >= 0]
if len(cross_valid) > 0:
    cross_counts = cross_valid["first_abs_cross_layer"].value_counts().sort_index().to_dict()
    cross_in_mid = float(np.mean(cross_valid["first_abs_cross_layer"].isin(MID_LAYERS)))
    cross_in_critical = float(np.mean(cross_valid["first_abs_cross_layer"].isin(CRITICAL_LAYERS)))
    cross_in_basin = float(np.mean(cross_valid["first_abs_cross_layer"].isin(BASIN_LAYERS)))
else:
    cross_counts = {}
    cross_in_mid = np.nan
...
# 3. Condition-level ordering of mid_abs_mean
condition_abs_order = (
    condition_feature_profile[
        [
            "condition",
    ...
    .sort_values("mid_abs_mean_7_19")
    .to_dict(orient="records")
)

existence_diagnostics = {
    "phase_profile": phase_profile.to_dict(orient="records"),
    "threshold_cross_counts_abs_gap_gt_1": {str(k): int(v) for k, v in cross_counts.items()},
    "fraction_first_abs_cross_in_mid_7_19": cross_in_mid,
    "fraction_first_abs_cross_in_critical_20_22": cross_in_critical,
    "fraction_first_abs_cross_in_basin_23_26": cross_in_basin,
    "condition_order_by_mid_abs_mean": condition_abs_order,
}
...
df.to_csv(SAVE_DIR / "ua1b_dataset.csv", index=False, encoding="utf-8-sig")
traj.to_csv(SAVE_DIR / "ua1b_gap_trajectory.csv", index=False, encoding="utf-8-sig")
feat.to_csv(SAVE_DIR / "ua1b_feature_table.csv", index=False, encoding="utf-8-sig")
condition_profile.to_csv(SAVE_DIR / "ua1b_condition_profile.csv", index=False, encoding="utf-8-sig")
condition_feature_profile.to_csv(SAVE_DIR / "ua1b_condition_feature_profile.csv", index=False, encoding="utf-8-sig")

with open(SAVE_DIR / "ua1b_prediction_summary.json", "w", encoding="utf-8") as f:
    json.dump(prediction_summary, f, ensure_ascii=False, indent=2)

summary = {
    "experiment": "UA-1B True Competing Basin Audit",
    "n_rows": int(len(df)),
    ...
    "observable": "Gap_l = logit_l(C) - logit_l(E), potential-gap proxy.",
    ...
    "interpretation_guide": {
        "PASS_strong": [
            "true_compete conditions have lower mid_abs_mean than positive/negative conditions",
            "phase_target_classification mid_only macro_f1 is meaningfully above chance",
            "mid_only predicts crit_mean/crit_abs_mean and basin_mean/basin_abs_mean",
            "positive/critical/negative trajectories separate as signed/near-zero/negative phases"
        ],
        "PASS_mixed": [
            "mid features predict later gaps but true_compete still biased to one side",
            "critical behavior emerges only around L20-L22, not in L7-L19"
        ],
        "FAIL": [
            "true_compete behaves like positive or negative, not near zero",
            "mid features do not predict later gap structure"
        ]
    }
}

with open(SAVE_DIR / "ua1b_summary.json", "w", encoding="utf-8") as f:
