...
def main():
    ...
    # If infer_pred_columns derived the true label from condition, make sure the
    # selected prediction frame actually receives those columns. The previous
    # version returned "policy_class" without mutating caller state.
    if true_col not in pred.columns:
        if "condition" in pred.columns:
            pred = assign_true_policy(pred)
            true_col = "policy_class"
        else:
            raise RuntimeError(f"Inferred true label column {true_col!r}, but it is not present and condition is unavailable.")
    if pred_col not in pred.columns:
        raise RuntimeError(f"Inferred predicted label column {pred_col!r}, but it is not present in predictions file.")
