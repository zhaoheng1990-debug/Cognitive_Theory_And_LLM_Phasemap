...
SCRIPT_VERSION="SOB-V6_FIXED_v2_LOW_STATES_PRECHECK"
...
def preflight_check():
    required = ["EXPERIMENT_ID", "OUT_DIR", "MODEL_SPECS", "MODELS_TO_RUN", "ROUTES", "RELEVANCE_WEIGHT", "LOW_STATES", "SUBJECT_STATES", "OBJECTS"]
    missing = [name for name in required if name not in globals()]
    if missing:
        raise RuntimeError("Missing required constants: " + ", ".join(missing))
    for sid, *_ in SUBJECT_STATES:
        if not isinstance(sid, str):
            raise RuntimeError("SUBJECT_STATES malformed: state id must be string")
