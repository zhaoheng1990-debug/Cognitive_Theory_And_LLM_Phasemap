...
# ============================================================
# ASA-9B.2 Driver: Binding-Aware Cross-Task Policy Audit
#
# Purpose:
#   ASA-9B-lite showed:
...
#
#   ASA-9B.2 adds override-aware submechanism labels while reusing the
#   ASA-8D engine:
#
#       stable
#       competition
...
# Outputs:
#   asa9b2_outputs/
#     asa9b2_cross_task_summary.csv
#     asa9b2_cross_task_summary.json
#     asa9b2_final_verdict.json
...
ROOT_SAVE_DIR = Path("./asa9b2_binding_aware_outputs")
...
# Binding-aware prompt-level features.
# Extracted from prompt text; not from ground-truth answer labels.
BINDING_FEATURE_COLS = [
    "bind_has_general_rule",
    "bind_has_override",
    "bind_has_exception",
    "bind_has_old_new_update",
    "bind_has_temporal_marker",
    "bind_has_source_registry",
    "bind_has_direct_conflict",
    "bind_has_competing_fact",
    "bind_has_irrelevant",
    "bind_override_exception_balance",
    "bind_update_strength",
    "bind_rule_strength",
    "bind_source_strength",
]

def extract_binding_features_from_text(text):
    s = str(text).lower()
    has_general = int(("general rule" in s) or ("rule:" in s) or ("rule a" in s) or ("default registry" in s) or ("registry rule" in s))
    has_override = int(("override" in s) or ("new rule" in s) or ("updated record" in s) or ("verified registry update" in s))
    has_exception = int(("exception" in s) or ("special case" in s) or ("exception list" in s) or ("exception entry" in s))
    has_update = int(("old" in s and "new" in s) or ("updated" in s) or ("now has" in s) or ("today" in s) or ("currently" in s))
    has_temporal = int(("time t1" in s) or ("time t2" in s) or ("yesterday" in s) or ("today" in s) or ("current" in s) or ("currently" in s) or ("latest" in s))
    has_source = int(("trusted registry" in s) or ("registry" in s) or ("source" in s) or ("record" in s) or ("memo" in s) or ("verified" in s))
    has_direct = int(("direct conflicting" in s) or ("direct statement" in s) or ("direct later note" in s) or ("direct exception candidate" in s))
    has_competing = int(("competing fact" in s) or ("may receive" in s) or ("record 1" in s and "record 2" in s) or ("another note" in s))
    has_irrelevant = int(("irrelevant" in s) or ("unrelated" in s))
    return {
        "bind_has_general_rule": has_general,
        "bind_has_override": has_override,
        "bind_has_exception": has_exception,
        "bind_has_old_new_update": has_update,
        "bind_has_temporal_marker": has_temporal,
        "bind_has_source_registry": has_source,
        "bind_has_direct_conflict": has_direct,
        "bind_has_competing_fact": has_competing,
        "bind_has_irrelevant": has_irrelevant,
        "bind_override_exception_balance": float(has_override + has_exception),
        "bind_update_strength": float(has_update + has_temporal + has_source),
        "bind_rule_strength": float(has_general + has_override + has_exception),
        "bind_source_strength": float(has_source + has_update),
    }

def attach_binding_features(feature_df, base_df):
    prompt_to_text = dict(zip(base_df["prompt_id"].astype(str), base_df["text"].astype(str)))
    rows = [extract_binding_features_from_text(prompt_to_text.get(str(pid), "")) for pid in feature_df["prompt_id"].astype(str)]
    bind_df = pd.DataFrame(rows).reset_index(drop=True)
    out = feature_df.reset_index(drop=True).copy()
    for col in BINDING_FEATURE_COLS:
        out[col] = bind_df[col].values if col in bind_df.columns else 0.0
    return out
...
def select_single_token_labels(tokenizer, save_dir):
    ...
    for lab in LABEL_CANDIDATES:
        ...
        if len(ids) == 1:
            chosen.append(lab)
    pd.DataFrame(rows).to_csv(Path(save_dir) / "asa9b2_label_tokenization_audit.csv", index=False, encoding="utf-8-sig")
    if len(chosen) < 12:
...
def build_override_aware_dataset(tokenizer, cfg, save_dir):
    ...
    df.to_csv(Path(save_dir) / "asa9b2_dataset.csv", index=False, encoding="utf-8-sig")
    return df
...
def patch_engine_for_six_mechanisms(engine):
    # Patch 6-way submechanism space.
    ...
    pred_mech_cols = [
        "p_mech_stable",
        "p_mech_competition",
        "p_mech_canonical_closure",
        "p_mech_temporal_update",
        "p_mech_rule_override",
        "p_mech_exception_binding",
        "pred_mech_stable",
        "pred_mech_competition",
        "pred_mech_canonical_closure",
        "pred_mech_temporal_update",
        "pred_mech_rule_override",
        "pred_mech_exception_binding",
    ]

    # Binding-aware policy columns.
    engine.POLICY_PRED_MECH_COLS = list(engine.MECH_FEATURE_COLS) + pred_mech_cols + list(BINDING_FEATURE_COLS)

    # Dataset builder.
    engine.build_dataset = build_override_aware_dataset

    # Attach prompt-binding features to state features before mechanism/policy training.
    _orig_base_state_features = engine.base_state_features_for_samples

    def _base_state_features_with_binding(base_df, h_by_layer, probes, vel_stats, sample_idx, cfg):
        out = _orig_base_state_features(base_df, h_by_layer, probes, vel_stats, sample_idx, cfg)
        return attach_binding_features(out, base_df)

    engine.base_state_features_for_samples = _base_state_features_with_binding

    # Robust predicted-mechanism attachment with all 6-way columns.
...
def ensure_six_mech_columns(df):
    """Ensure predicted-mechanism feature table has all 6-way probability/one-hot columns.

    sklearn classifiers may not expose probability columns for classes absent
    in a LOTFO train split. We fill absent classes with 0.0.
    """
    out = df.copy()
...
def make_lotfo_split_func(heldout_family):
    def lotfo_split(df, cfg, save_dir):
        ...
        split_df.to_csv(Path(save_dir) / "asa9b2_lotfo_split.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    print(f"[ASA-9B.2] Using ASA-8D engine: {engine_file}")
    ...
    for heldout in TASK_FAMILIES_TO_HOLDOUT:
        print("\n" + "=" * 88)
        print(f"[ASA-9B.2] Override-aware LOTFO heldout = {heldout}")
        print("=" * 88)
        ...
        with open(save_dir / "asa9b2_run_config.json", "w", encoding="utf-8") as f:
            json.dump({
                "model_key": MODEL_KEY,
        ...
        try:
            ...
            print(f"[ASA-9B.2][ERROR] heldout={heldout}: {repr(e)}")
            traceback.print_exc()
            row = {
                "heldout_task_family": heldout,
                "status": "error",
            ...
            all_rows.append(row)
            with open(save_dir / "asa9b2_error.json", "w", encoding="utf-8") as f:
                json.dump(row, f, indent=2, ensure_ascii=False, default=to_jsonable)

    summary_df = pd.DataFrame(all_rows)
    summary_df.to_csv(ROOT_SAVE_DIR / "asa9b2_cross_task_summary.csv", index=False, encoding="utf-8-sig")

    with open(ROOT_SAVE_DIR / "asa9b2_cross_task_summary.json", "w", encoding="utf-8") as f:
    ...
    final = {
        "audit": "ASA-9B.2 Binding-Aware Cross-Task Policy Audit",
        "model": MODEL_KEY,
        "task_families": TASK_FAMILIES,
        "task_families_to_holdout": TASK_FAMILIES_TO_HOLDOUT,
        "mechanism_space": MECH_TO_ID_6,
        ...
        "n_shuffles": N_SHUFFLES,
        "summary_csv": "asa9b2_cross_task_summary.csv",
        "interpretation_rules": {
            "exception_improves": "Binding-aware prompt features repair the remaining exception/override Lite bottleneck.",
            "temporal_remains_pass": "Temporal update remains transferable under finer mechanism space.",
            "still_fails_exception": "Need non-textual binding probes, not only prompt keyword binding features.",
        },
    }

    with open(ROOT_SAVE_DIR / "asa9b2_final_verdict.json", "w", encoding="utf-8") as f:
