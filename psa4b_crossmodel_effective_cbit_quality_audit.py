# -*- coding: utf-8 -*-
"""
PSA-4B: Cross-Model Effective Cbit -> Cognitive Quality Replication
===================================================================

Purpose
-------
PSA-4 on Qwen showed:

    stagewise Cbit > direct Cbit
    effective Cbit improves answer-margin prediction
    Cbit predicts cognitive quality

PSA-4B tests whether this replicates across Qwen / Llama / Gemma.

Main claims
-----------
1. Quality is non-degenerate in each model.
2. Stagewise Cbit predicts answer quality better than direct Cbit.
3. Effective Cbit improves continuous answer-margin prediction.
4. Cross-model aggregate supports:
       PossibilitySpace -> Cbit -> EffectiveCbit -> CognitiveQuality
...
Run smoke one model:
    python psa4b_crossmodel_effective_cbit_quality_audit.py --models qwen --n_base 40 --topk 30

Run formal all models:
    python psa4b_crossmodel_effective_cbit_quality_audit.py --models qwen llama gemma --n_base 120 --topk 50
"""
...
DEFAULT_OUT_DIR = r"C:\Users\ZH\Desktop\AGI\outputs\psa4b_crossmodel_outputs"
SEED = 20260607
...
def sanitize_array(x, name="array"):
    """
    Replace NaN/Inf before sklearn/PCA.
    This is required for cross-model runs because some models/layers can produce
    zero-norm TopK centers, fp16 overflow/underflow artifacts, or empty-window
    derived values that sklearn refuses to process.
    """
    arr = np.asarray(x)
...
def run_one_model(model_key, args, prompt_df, pair_df, model_out_dir):
    ...
    quality.to_csv(model_out_dir / f"psa4b_{model_key}_answer_quality.csv", index=False, encoding="utf-8-sig")

    # Free GPU memory before metrics if possible later outside
    ...
    pair_local = sanitize_dataframe_numeric(pair_local, f"{model_key}.pair_local_after_quality")
    ...
    pair_local = sanitize_dataframe_numeric(pair_local, f"{model_key}.pair_local_final")
    pair_local.to_csv(model_out_dir / f"psa4b_{model_key}_pairs_with_cbit_quality.csv", index=False, encoding="utf-8-sig")

    y_margin = sanitize_array(pair_local["answer_margin"].values.astype(float), f"{model_key}.answer_margin")
    ...
    correct_rate = float(pair_local["answer_correct"].mean())
    ...
    if quality_degenerate:
        ...
        verdict = "PASS_PSA4B_EFFECTIVE_CBIT_PREDICTS_QUALITY"
    elif stage_predicts and stage_beats_direct:
        verdict = "PARTIAL_PSA4B_STAGEWISE_CBIT_PREDICTS_QUALITY"
    elif stage_predicts:
        verdict = "PARTIAL_PSA4B_CBIT_QUALITY_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4B"
    ...
    out = {
        ...
        "summary_metrics": {
            ...
            "answer_margin_mean": float(pair_local["answer_margin"].mean()),
            "answer_margin_std": float(pair_local["answer_margin"].std()),
    ...
    with open(model_out_dir / f"psa4b_{model_key}_summary.json", "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    with open(model_out_dir / f"psa4b_{model_key}_verdict.txt", "w", encoding="utf-8") as f:
...
def aggregate_results(model_summaries):
    ...
    n = len(df)
    pass_like = df["verdict"].str.contains("PASS|PARTIAL", regex=True).sum()
    ...
    if pass_like == n and stage_beats >= max(1, n - 1) and stage_predicts >= max(1, n - 1):
        verdict = "PASS_PSA4B_CROSSMODEL_STAGEWISE_CBIT_REPLICATION"
    elif stage_predicts >= max(1, n - 1):
        verdict = "PARTIAL_PSA4B_CROSSMODEL_CBIT_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4B_CROSSMODEL"
    ...
    return df, {
        ...
        "models": rows,
        "interpretation": (
            "PSA-4B tests cross-model replication of the PSA-4 claim: stagewise structural Cbit "
            "should predict cognitive quality better than direct Cbit, and effective Cbit should improve "
            "quality prediction in at least a subset of models."
        ),
...
def main():
    ...
    prompt_df.to_csv(out_dir / "psa4b_prompts.csv", index=False, encoding="utf-8-sig")
    pair_df.to_csv(out_dir / "psa4b_pairs.csv", index=False, encoding="utf-8-sig")
    ...
    agg_df.to_csv(out_dir / "psa4b_crossmodel_summary_table.csv", index=False, encoding="utf-8-sig")

    agg["config"] = vars(args)
    with open(out_dir / "psa4b_crossmodel_summary.json", "w", encoding="utf-8") as f:
        json.dump(agg, f, ensure_ascii=False, indent=2)
    with open(out_dir / "psa4b_crossmodel_verdict.txt", "w", encoding="utf-8") as f:
