...
\section{Discussion}
Across the tested checkpoints, realized transitions formed a measurable internal process, partial trajectories forecast a later candidate margin, and local perturbations crossed a declared emitted-token boundary. Components specify implementation; the ordered trajectory supplies the empirical object on which computation can be compared, forecast and perturbed. The result is a dynamics-level causal account with bounded behavioural leverage, supported by observation, reduced coordinates, held-out prediction and intervention on the same process.
