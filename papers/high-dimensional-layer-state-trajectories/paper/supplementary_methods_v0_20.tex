...
\usepackage{enumitem,seqsplit,fvextra}
\usepackage{titlesec,fancyhdr,hyperref,xurl}
...
\raggedbottom
\titleformat{\section}{\normalfont\large\bfseries\sffamily}{\thesection}{0.45em}{}
\titleformat{\subsection}{\normalfont\normalsize\bfseries\sffamily}{\thesubsection}{0.45em}{}
\titlespacing*{\section}{0pt}{1.2em}{0.45em}
\titlespacing*{\subsection}{0pt}{0.9em}{0.3em}
\pagestyle{fancy}
\fancyhf{}
\lhead{\sffamily\footnotesize Supplementary Methods v0.20}
\rhead{\sffamily\footnotesize LLM dynamics manuscript}
\cfoot{\sffamily\thepage}
\begin{document}
\begin{center}
{\Large\bfseries\sffamily Supplementary Methods: object-switch trajectory analysis and human-AI workflow}\\[0.5em]
{\normalsize Heng Zhao, Yufei Wang, Ruidian Song and Weisheng Chen}
\end{center}
\section*{1. Empirical-object hierarchy}

The study separates three levels that are easily conflated.

\begin{enumerate}[leftmargin=1.6em,itemsep=0.2em,topsep=0.2em]

\item \textbf{Implementation objects:} model weights, attention heads, MLPs, residual blocks and layers. These are the engineered objects that realize inference.

\item \textbf{Empirical process object:} the ordered hidden-state sequence \(T_{m,p}=(h_{m,p,1},\ldots,h_{m,p,L_m})\). This is the object on which the present mechanism comparisons are defined.

\item \textbf{Coordinates and diagnostics:} direct hidden summaries, hidden-state flow, vocabulary-facing TopK/VIM readbacks, the decision-window clean-relative margin profile, \(\Delta U\), transition coordinates and residual diagnostics.
...
\section*{2. Intellectual provenance and operational boundaries}
...
\section*{3. Controlled prompt subset}
...
\textbf{Supplementary Methods Table 1 | Analysis-label and information-boundary crosswalk.}
...
Internal-control and confidence-gated intervention & Separate ten-condition panel. Stable: clean, redundant, irrelevant and paraphrase. Competition: weak distractor, balanced competition and direct conflict. Closure: closure update, closure override and exception override. & Three-class intervention label used by the prompt-condition classifier and gate & Training-graph prompt labels and pre-intervention trajectory features & Held-out outcomes, generated answer and correctness \\
Policy supervision & Eight-action internal library or expanded ten-action boundary library & Discrete \((\alpha,\beta)\) action maximizing the declared training utility; \texttt{none} is \((0,0)\) & Training-graph intervention responses and fitted mechanism features & Held-out action outcomes and correctness \\
...
\section*{4. Direct hidden-state update summaries for Fig. 1}
...
\section*{5. Readback and projection-selection controls}
...
\textbf{Algorithm 1: independent coordinate audit}
...
for each checkpoint and proxy family:
    ...
    for each held-out fold:
        ...
        fit the base prompt-condition classifier on predecision state features
...
\section*{6. Exploratory functional-window scan and prospective boundary test}
...
\textbf{Algorithm 2: exploratory functional registration}
...
\subsection*{6.1 Prospective confirmation protocol and result}
...
\textbf{Supplementary Methods Table 2 | Prospective functional-window confirmation on 24 untouched graph groups.}
...
\section*{7. Executable \(\Delta U\) definition}
...
\section*{8. Transition-coordinate closure}
...
\section*{9. Direct hidden-state chord geometry}
...
\textbf{Algorithm 3: endpoint-preserving layer-order null}
...
\section*{10. Independent task-family and random-initialization controls}

\subsection*{10.1 Controlled lexical and addition tasks}
...
\section*{11. Auxiliary readback-centre geometry}
...
\section*{12. Intervention labels, features and splits}
...
and \texttt{none} denotes \((0,0)\). For training prompts, closure labels maximize signed \(\Delta U\) shift; stable and competition labels minimize absolute shift. The final policy is compared with matched fixed combinations and 50 prompt-condition-label, policy-label and joint shuffles.

The prompt-condition classifier (labelled \texttt{mechanism classifier} in archived code and Fig. 5d) used the nine-dimensional vector

\begin{equation*}
x^{\mathrm{pc}}=(\widehat{\Delta U},|\widehat{\Delta U}|,\widehat D_B,\lVert v\rVert,z_v,\tilde l,I_{\mathrm{precursor}},\Delta U_{\mathrm{base}},D_{B,\mathrm{base}}).
\end{equation*}

It was a 250-tree \texttt{RandomForestClassifier} with \(max_depth=7\), \(min_samples_leaf=6\), \(class_weight=balanced_subsample\) and seed \(42+s\), where \(s=0\) for the real fit and the shuffle index otherwise. The policy input appended the three predicted prompt-condition probabilities and three one-hot predicted-class indicators, giving \(x^{\mathrm{policy}}\in\mathbb R^{15}\). The policy was a 200-tree random forest with \(max_depth=6\), \(min_samples_leaf=8\), the same class weighting and seed \(142+s\). Both used the scikit-learn default weighted-Gini split criterion. They contained no neural hidden layers and used neither gradient descent nor policy gradients.
...
The prompt-condition-label null permutes training prompt-condition labels, refits the prompt-condition classifier and then refits the downstream policy. The policy-label null retains the real prompt-condition classifier but permutes training action labels before policy fitting. The joint null independently permutes both label sets and refits both classifiers. All nulls retain the graph split, test rows, transition operators, action grid and endpoint calculation.

\textbf{Executable procedure: held-out mechanism-conditioned intervention}

\begin{Verbatim}[fontsize=\small,breaklines=true,breakanywhere=true]

split complete relation graphs 70:30 into training and held-out sets

fit transition operators, prompt-condition classifier and action policy on training graphs

for each held-out prompt and selected layer:

    predict prompt-condition probabilities
...
\subsection*{Out-of-fold margin-shift and boundary-crossing analysis}
...
Five-fold \texttt{GroupKFold} holds out complete relation graphs, with the same graph held out across checkpoints. The predicted post-policy margin is \(\widehat M_{p,\mathrm{policy}}=M_{p,0}+\widehat{\delta M}_p\). For the at-risk population \(M_{p,0}<0\), the observed crossing label is \(I(M_{p,\mathrm{policy}}\geq0)\). Within each outer fold, an inner graph cross-fit supplies training predictions for a one-variable logistic calibrator, which maps predicted post-margin to crossing probability. Thus neither the margin-shift prediction nor its probability calibration evaluates a graph used for fitting.
...
\textbf{Supplementary Methods Table 3 | Confidence-gated actuator component attribution.}
...
\textbf{Executable procedure: confidence-gated local transition control}
...
\section*{13. Human-AI hypothesis-to-audit workflow}
...
\item \textbf{Human question or contradiction:} identify a mechanistic question, evidentiary gap or failed interpretation.

\item \textbf{Executable proposal:} use AI assistance to translate the question into candidate metrics, controls, scripts or consistency checks.

\item \textbf{Local execution:} run analyses against stated checkpoints and archived data.

\item \textbf{Evidence audit:} compare outputs with leakage restrictions, nulls, source files and rival explanations.

\item \textbf{Human decision:} retain, revise or reject the object, metric, interpretation or claim.

\item \textbf{Trace-preserving iteration:} archive scripts, outputs, negative results and the reason for the next test.
...
\section*{14. Responsibility matrix}
...
\section*{15. Research independence and disclosure completeness}
...
\section*{16. Additional architecture, order, metric and training controls}

\subsection*{16.1 Random fixed-map relational continuity}
...
\subsection*{16.2 Reordered execution and functional fidelity}
...
\subsection*{16.3 Incremental path-history prediction}
...
\subsection*{16.4 Radial sensitivity of the Llama statistic}
...
\subsection*{16.5 Pythia component compatibility}
...
\subsection*{16.6 Training-time task coordinates}
...
\textbf{Supplementary Methods Table 4 | Claim-boundary audit.}
...
\textbf{Supplementary Methods Table 5 | Held-out closure-prompt boundary outcomes under confidence-gated control.}
...
\begin{landscape}
\textbf{Supplementary Methods Table 6 | Frozen mixed-arithmetic primary actuator result.}
...
\end{longtable}
\vspace{1.2em}
\textbf{Supplementary Methods Table 7 | Frozen mixed-arithmetic actuator-component controls.}
...
\begin{landscape}
\textbf{Supplementary Methods Table 8 | Experiment subset, window and null-control metadata.}
...
Predecision coordinate competition & 96 relation-graph groups per checkpoint & Frozen predecision cutoffs: Qwen 22, Llama 13 and Gemma 20 & Five GroupKFold partitions; 100,000 graph-level sign flips; BH across 9 contrasts & OOF prompt-condition-label macro-F1 against matched recent logit margins \\
...
\section*{17. Frozen closure audits and claim-family multiplicity}
...
\textbf{Supplementary Methods Table 9 | Frozen predecision coordinate competition.}
...
\subsection*{17.2 Norm-, layer- and gate-matched empirical direction null}
...
\textbf{Supplementary Methods Table 10 | Empirical-subspace random-direction actuator null.}
...
\subsection*{17.3 New-graph paired geometry confirmation}

The 24 new graph groups used disjoint entity names, relation phrases and candidate labels from the original controlled set. Each group supplied the fixed ten condition types \texttt{clean}, \texttt{permuted}, \texttt{redundant}, \texttt{irrelevant}, \texttt{\seqsplit{weak\_distractor}}, \texttt{\seqsplit{competition\_balanced}}, \texttt{\seqsplit{direct\_conflict}}, \texttt{\seqsplit{closure\_update}}, \texttt{\seqsplit{closure\_override}} and \texttt{\seqsplit{exception\_override}}. The exact raw prompt string and prompt content digest were shared across the Qwen and Gemma runtimes. Geometry used all block outputs, with no functional window selection. Within each shuffle repeat one permutation of intermediate block indices was shared across all 240 prompts and endpoints were retained.

The Gemma checkpoint is a later-generation Gemma-3-12B instruction-tuned QAT Q4\_0 GGUF checkpoint rather than a same-family controlled scale ablation. Its native capture was qualified by two byte-identical 48-block exports before the full run. The primary result only says that the measured all-layer path geometry recurs in these two checkpoint-specific settings.

\textbf{Supplementary Methods Table 11 | Frozen paired new-graph chord-geometry confirmation.}
...
\setlength{\tabcolsep}{2.5pt}
\renewcommand{\arraystretch}{1.08}
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}>{\RaggedRight\arraybackslash}p{0.100\linewidth}@{}}
\toprule
...
Gemma-3-12B-it-QAT-Q4\_0 & 240 & 48 / 3,840 & 0.08640 & 0.02403 & 0.01839 & 3.392 & 0.0196 & 0.0392 \\
\end{longtable}
\end{landscape}
\end{document}
