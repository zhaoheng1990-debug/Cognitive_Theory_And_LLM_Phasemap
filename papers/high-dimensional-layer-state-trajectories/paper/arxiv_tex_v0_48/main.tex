...
\section{Discussion}
This study reveals an experimentally grounded picture of the dynamics inside transformer inference. Candidate trajectories compete as hidden states evolve through depth. Their motion is globally organized, geodesic-biased rather than exactly geodesic, and accompanied by structured off-direction processing. The same dynamical form recurs across Qwen, Llama and Gemma checkpoints, while its magnitude varies. Trajectory-informed interventions further show that this organization is operationally discriminating. These findings provide a mechanism-level account of the internal inference process, although they do not yet constitute a complete dynamical law.

Vocabulary readbacks remain useful, but their role is now clearer. They provide a common token-facing view across layers and reveal reproducible change with depth, extending intermediate vocabulary projections as diagnostic readouts\textsuperscript{11,20}. The row-permutation and Gaussian controls separate that usefulness from a stronger mechanism claim. Geometry survived when learned row identity was removed, while conditional selection remained important. The retained structure therefore belongs to the projection-selection process and the hidden states it samples, not to the semantic identity of individual output rows. Readbacks are appropriate for intermediate token content and rapid layerwise inspection. Claims about transition geometry require the ordered hidden states themselves.

The failed operator interpretation also produced a positive result. Fine K=8 labels largely encoded layer depth, which ruled out a small catalogue of reusable primitive operators in the tested representation. A coarse late transition nevertheless survived changes in clustering resolution and several robustness checks. This distinction matters: transformer depth supports organized change without requiring a discrete operator alphabet. It also explains why nominal layer matching was inadequate across checkpoints and why objective-selected functional windows became necessary.

Functional windows exposed recurrence without coordinate identity. Comparable roles appeared at different absolute depths, while condition-direction identity was weak. The commonality lies in objective-specific organization rather than a shared basis or fixed layer number. This provides a practical route for comparing checkpoints with different depths and hidden dimensions. It also changes the object of cross-model comparison: the question becomes whether a functional relation recurs, not whether two layers have identical coordinates. Independent selection and confirmation of the windows remains necessary before treating their boundaries as stable checkpoint properties.

The low-rank dominance coordinate \(\Delta U\) then compressed candidate-relative organization within those windows. Its value is not that it names a universal order parameter. It supplies a reproducible scalar question about which of two declared candidate trajectories has greater relative support. Predecision exclusion, non-R predictors and graph-grouped resampling showed that related organization appears before the defining decision window. The construction therefore captures more than a direct rereading of the target window, while remaining tied to the controlled candidate-pair design.

Observed-transition coordinates and route geometry reveal two levels of dynamics. Coordinates computed from a realized transition accounted for most of the adjacent-state relation, but advance estimation recovered only a small part of that gain. This separates descriptive transition closure from prediction and identifies the missing information that a forward model must recover. At the route level, real layer order was more aligned and less curved or detoured than order shuffles in all three checkpoints. Large residual ratios ruled out exact-geodesic dynamics. Residual features nevertheless retained candidate-dominance and mechanism-label information and complemented the reference-aligned component. Transformer inference in these experiments is therefore neither an arbitrary walk nor an exact shortest path. It is an organized, geodesic-biased flow with structured off-direction motion.

The structured residual is also a constructive limitation of the present coordinate system. A reference direction and its orthogonal complement do not form a complete local account of hidden-state motion. Exploratory correction measures were more stable than rotation measures, consistent with off-direction motion that partly restores candidate-relative progress. That interpretation remains coordinate dependent. Richer local bases and transport-aware decompositions can now be judged by a concrete criterion: they should explain residual structure without losing the cross-window and cross-checkpoint organization established here.

The cross-checkpoint results separate recurrence from effect magnitude. Qwen, Llama and Gemma all showed ordered trajectory organization relative to their controls, but they did not yield a common numerical effect. Llama had weaker predecision association, little matched-capacity advantage for \(\Delta U\) at some retained dimensions and a smaller raw action-specificity gain than Gemma. Its joint-shuffle result remained significant, whereas one pairing-shuffle comparison did not. These differences rule out a checkpoint-independent effect size and make checkpoint sensitivity part of the finding. Depth, attention configuration, training and vocabulary all vary together in the present comparison, so their individual contributions require controlled model interventions rather than post-hoc attribution.

The mechanism-aware policy experiment gives the trajectory coordinates operational significance. Labels were fixed by controlled prompt condition, and the policies acted on held-out relation graphs. Trajectory-informed actions produced more target-specific internal shifts than fixed actions and several label-shuffled controls. This is not merely a restatement of correlation: the coordinate distinguished matched from mismatched actions at the intervention endpoint. The experiment therefore closes a controlled observation-to-operation loop. A behavioural extension should test whether these internal shifts predict or mediate candidate selection and answer correctness.

The present evidence comes from three instruction-tuned checkpoints in the 1B-2B parameter range and from controlled candidate-competition prompts. It does not yet establish the same organization in larger base models, multimodal transformers or open-ended generation. Functional windows were selected and evaluated in the same controlled dataset, and the order-shuffle null does not match every local smoothness statistic. The analysis describes trajectory geometry rather than deriving a numerically simulable differential equation. Its operational endpoint is internal rather than behavioural. These are targeted experimental boundaries, not reasons to retreat from the retained conclusions. Independent window confirmation, smoothness-matched route nulls, broader model and prompt regimes, and behavioural endpoints would test how far the trajectory account extends.

The broader dynamical landscape includes how training shapes the available representational geometry and how prompts set the initial state and direction of inference. Those upstream processes were not measured here. The present experiments reveal the inference-stage mechanism between them: an ordered high-dimensional competitive trajectory whose organization survives falsification of vocabulary identity, recurs across distinct checkpoints, contains structured non-geodesic motion and supports target-specific internal modulation. This is an incomplete but substantive picture of how the LLM black box evolves internally during inference, and a concrete basis for testing fuller dynamical theories.
...
The interpretation follows five constraints. \(\Delta U\) is a bounded dominance observable. Vocabulary readbacks are diagnostics. \(O_l^{\mathrm{adv}}\) and \(O_l^{\mathrm{cont}}\) denote different quantities. Low-dimensional plots are orientation aids. The trajectory-geometry and internal-modulation analyses support geodesic-biased flow, structured residuals and internal modulation only within their declared endpoints.
...
Processed panel-level tables for the main and Supplementary Figures are archived on Zenodo at \url{https://doi.org/10.5281/zenodo.21317356} and are also available in the public project repository:

Https://github.com/zhaoheng1990-debug/Cognitive\_Theory\_And\_LLM\_Phasemap

The release contains prompt and checkpoint metadata, hidden-state extraction records, random seeds, null-control specifications, classifier definitions, figure contracts and export checks. Raw hidden-state arrays are large derivatives of third-party checkpoints and are not included in the current release. The prompts, checkpoint revisions and extraction scripts required to regenerate them are provided, subject to checkpoint access, storage and licensing constraints. Model weights are not redistributed.

\section{Code availability}
Analysis scripts, R figure scripts, environment records, smoke tests and portable path templates are archived on Zenodo at \url{https://doi.org/10.5281/zenodo.21317441} and are available in the same public reproducibility branch. Workstation-specific paths found in the original analyses have been replaced or wrapped with repository-relative arguments while provenance-preserving filenames are retained. Full experiment reruns require the listed model checkpoints and any upstream datasets that cannot be redistributed. The Zenodo records and GitHub branch are linked in the release metadata.
