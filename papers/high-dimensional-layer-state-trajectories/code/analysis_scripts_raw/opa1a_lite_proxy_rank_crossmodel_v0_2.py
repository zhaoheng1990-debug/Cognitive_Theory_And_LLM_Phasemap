...
#
# Default input:
#   inputs/opa1a_lite_answer_pair_crossmodel_safe_v0_2.csv
#
# Default output:
#   opa1a_lite_crossmodel_outputs/
#
# ============================================================
...
try:
    ...
    PCA = None
    accuracy_score = f1_score = r2_score = roc_auc_score = None
    GroupKFold = Ridge = LogisticRegression = StandardScaler = Pipeline = None
...
RUN_MODEL_KEYS = ["qwen", "llama", "gemma"]

PROMPT_CSV = Path("inputs/opa1a_lite_answer_pair_crossmodel_safe_v0_2.csv")
SAVE_DIR = Path("opa1a_lite_crossmodel_outputs")
...
def parse_args():
    parser = argparse.ArgumentParser(description="OPA-1A-lite ontology proxy rank audit")
    parser.add_argument("--prompt-csv", type=Path, default=PROMPT_CSV)
    ...
    parser.add_argument("--models", default=",".join(RUN_MODEL_KEYS), help="Comma-separated model keys")
    parser.add_argument("--device", default=DEVICE, choices=["auto", "cpu", "cuda"])
    parser.add_argument("--seed", type=int, default=SEED)
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE)
    parser.add_argument("--max-len", type=int, default=MAX_LEN)
    parser.add_argument("--topk-k", type=int, default=TOPK_K)
    parser.add_argument("--gaussian-seed", type=int, default=GAUSSIAN_SEED)
    parser.add_argument("--use-chat-template", action="store_true", default=USE_CHAT_TEMPLATE)
...
def configure_from_args(args):
    global PROMPT_CSV, SAVE_DIR, MODEL_SPECS, RUN_MODEL_KEYS, DEVICE, DTYPE
    global SEED, BATCH_SIZE, MAX_LEN, TOPK_K, GAUSSIAN_SEED, USE_CHAT_TEMPLATE
    ...
    RUN_MODEL_KEYS = [x.strip() for x in str(args.models).split(",") if x.strip()]
    if args.device == "auto":
        DEVICE = "cuda" if torch is not None and torch.cuda.is_available() else "cpu"
    else:
        DEVICE = args.device
...
def write_input_check_report():
    ...
    report = {
        "script": "opa1a_lite_proxy_rank_crossmodel_v0_2.py",
        ...
        "inputs": {
            "prompt_csv": path_status(PROMPT_CSV),
            "model_paths": {name: path_status(path) for name, path in MODEL_SPECS.items()},
        },
        "outputs": {"save_dir": path_status(SAVE_DIR)},
    }
    report_path = SAVE_DIR / "opa1a_lite_input_check_report.json"
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"OPA-1A-lite input check report written to: {report_path}")

def ensure_runtime_dependencies():
    missing = [name for name, ok in dependency_status().items() if not ok]
    if missing:
        raise RuntimeError(
            "Missing required runtime dependencies for full OPA-1A-lite run: "
            + ", ".join(missing)
...
def load_tokenizer(path):
    if AutoTokenizer is None:
        raise RuntimeError("transformers is required for full OPA-1A-lite runs.")
...
def load_model(path):
    if AutoModelForCausalLM is None:
        raise RuntimeError("transformers is required for full OPA-1A-lite runs.")
    if torch is None:
        raise RuntimeError("torch is required for full OPA-1A-lite runs.")
...
def ridge_group_cv(X, y, groups, n_splits=5):
    if Pipeline is None:
        raise RuntimeError("scikit-learn is required for full OPA-1A-lite runs.")
...
def proxy_eval_rows(model_key, proxy_name, X, deltaU, phase_y, mechanism_y, groups):
    ...
    return {
        ...
        "deltaU_corr": corr,
        "phase_acc": phase_acc,
        "phaçÍ-¢G§²ÚîÆ­yÝodel(QWEN_PRECURSOR_LAYERS, num_layers)
