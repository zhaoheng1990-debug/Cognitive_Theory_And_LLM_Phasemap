...
try:
    ...
    ColumnTransformer = SimpleImputer = Ridge = None
    r2_score = mean_absolute_error = mean_squared_error = None
    GroupKFold = KFold = Pipeline = None
    OneHotEncoder = StandardScaler = PolynomialFeatures = None
...
# Default paths
# --------------------------
INPUT_CSV = Path("inputs/phasemap6b1_outputs/phasemap6b1_correction_dataset.csv")
...
def parse_args():
    parser = argparse.ArgumentParser(description="PhaseMap-7E state-equation ablation audit")
...
def configure_from_args(args):
    global INPUT_CSV, OUTPUT_DIR, RANDOM_SEED, N_SPLITS, RIDGE_ALPHA
...
def write_input_check_report():
    ...
    report = {
        "script": "GPT_145_phasemap7e.py",
        "audit": "PhaseMap-7E State Equation Ablation Audit",
        ...
        "outputs": {"output_dir": path_status(OUTPUT_DIR)},
    }
    report_path = OUTPUT_DIR / "phasemap7e_input_check_report.json"
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"PhaseMap-7E input check report written to: {report_path}")

def ensure_runtime_dependencies():
    if Pipeline is None:
        raise RuntimeError(
            "Missing required runtime dependency for full PhaseMap-7E run: scikit-learn. "
