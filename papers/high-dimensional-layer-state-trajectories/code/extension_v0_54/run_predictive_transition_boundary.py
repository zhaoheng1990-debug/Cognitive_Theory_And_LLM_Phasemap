# ============================================================
# Transition-predictive bilinear closure v0.1
# O-cont origin and target-excluded prediction audit
#
# Goal:
#   The descriptive closure analysis showed O_cont is crucial for:
#
...
#
#   This audit tests whether O_cont can be generated from pre-transition
#   information only:
#
...
#
# Fixed input:
#   transition_closure_dataset.csv
#
# Outputs:
#   predictive_transition_outputs/
# ============================================================
...
DEFAULT_DATA = HERE.parents[1] / "source_data" / "extension_v0_54" / "predictive_transition_boundary" / "transition_closure_dataset.csv"
INPUT_CSV = os.environ.get("TRANSITION_DATASET", str(DEFAULT_DATA))
OUTPUT_DIR = Path(os.environ.get("TRANSITION_OUTPUT_DIR", str(HERE / "outputs" / "predictive_transition_boundary")))
...
def fit_predict_infold_two_stage(df, o_feature_cols, z_feature_builder, op_cols, z_target_cols):
    ...
    for fold_idx, (tr, te) in enumerate(splits):
        ...
        # Build interaction features independently from each fold's own
        # target-excluded O_hat values. The historical implementation built
        # them only on train_aug, so the test-frame lookup failed.
        train_z_features = z_feature_builder(train_aug, prefix="ohat_")
        test_z_features = z_feature_builder(test_aug, prefix="ohat_")

        if train_z_features != test_z_features:
            raise ValueError(
                "Train/test feature builders returned different feature sets: "
                f"train={train_z_features}, test={test_z_features}"
            )

        z_features = train_z_features
        missing_train = [c for c in z_features if c not in train_aug.columns]
        missing_test = [c for c in z_features if c not in test_aug.columns]
        if missing_train or missing_test:
            raise KeyError(
                "Fold-local feature construction incomplete: "
...
if not Path(INPUT_CSV).exists():
    raise FileNotFoundError(
        ...
        f"transition_closure_dataset.csv and run again."
    )
...
if not op_cols:
    raise RuntimeError(
        "No op_pc* columns found. This audit requires continuous operator coordinates."
    )
...
g_summary.to_csv(OUTPUT_DIR / "operator_origin_prediction_summary.csv", index=False)
two_stage_summary.to_csv(OUTPUT_DIR / "two_stage_prediction_summary.csv", index=False)
fold_summary.to_csv(OUTPUT_DIR / "grouped_fold_summary.csv", index=False)
pred_rows.to_csv(OUTPUT_DIR / "out_of_fold_predictions.csv", index=False)

with open(OUTPUT_DIR / "predictive_transition_summary.json", "w", encoding="utf-8") as f:
    json.dump(summary, f, ensure_ascii=False, indent=2)

print("\n================ Predictive transition-boundary summary ================")
