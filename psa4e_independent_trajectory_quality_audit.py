# -*- coding: utf-8 -*-
"""
PSA-4E: Independent TrajectoryQuality Audit
===========================================

Why this version exists
-----------------------
PSA-4D showed strong cross-model Cbit -> TrajectoryQuality, but the TQ proxy was
partly constructed from Cbit/transition variables. That is useful as a coordinate
closure, but not enough as an independent quality test.

PSA-4E builds independent trajectory-quality proxies that do not directly use
transition Cbit formulas as target labels.

Independent TQ targets
----------------------
1. topk_flow_smoothness:
       inverse variation of TopK center movement across late trajectory
2. topk_entropy_stabilization:
       entropy drop / low late entropy from entry -> final
3. commit_final_set_stability:
       Jaccard/weighted-Jaccard stability of TopK token sets from commit -> final
4. cross_surface_family_consistency:
       within task_family consistency of final family
5. answer_rank_stability:
       correct endpoint rank quality under scoring
       (answer-adjacent but not in Cbit formula)
6. composite_independent_tq:
       average z-score of the above independent proxies

Main tests
----------
A. stagewise Cbit predicts independent TQ.
B. stagewise Cbit beats direct Cbit for independent TQ.
C. independent TQ predicts answer quality.
D. Cbit + independent TQ improves answer-quality prediction.

Run smoke:
    python psa4e_independent_trajectory_quality_audit.py --models qwen --n_base 40 --topk 30

Formal:
    python psa4e_independent_trajectory_quality_audit.py --models qwen llama gemma --n_base 120 --topk 50 --window_mode scan
"""
...
DEFAULT_OUT_DIR = r"C:\Users\ZH\Desktop\AGI\outputs\psa4e_independent_tq_outputs"
SEED = 20260607
...
def zscore(x):
    x = sanitize_array(np.asarray(x, dtype=float), "zscore")
...
@torch.no_grad()
def extract_hidden_topk_sets(tokenizer, model, df, device, max_len=256, topk=50, batch_size=1):
    all_hidden, all_ent, all_spread, all_center, all_ids, all_probs = [], [], [], [], [], []
    ...
    for start in range(0, len(df), batch_size):
        ...
        for h in hidden_states:
            ...
            li.append(ids.detach().cpu())
            lp.append(probs.detach().cpu())
        ...
        all_ids.append(torch.stack(li, dim=0).transpose(0, 1))
        all_probs.append(torch.stack(lp, dim=0).transpose(0, 1))
    ...
    feats = {
        ...
        "topk_ids": torch.cat(all_ids, dim=0).numpy(),
        "topk_probs": torch.cat(all_probs, dim=0).numpy(),
    }
    return {k: sanitize_array(v, f"extract.{k}") for k, v in feats.items()}
...
def build_delta_features(feats, pair_df):
    ...
    out = {
        ...
        "topk_ids_cond": feats["topk_ids"][cond],
        "topk_probs_cond": feats["topk_probs"][cond],
    }
...
def make_windows(L, profile="relative_default"):
    profiles = {
        "relative_default": {"shape": (0.30, 0.65), "entry": (0.65, 0.78), "commit": (0.78, 0.90), "final": (0.90, 1.00)},
        "relative_early": {"shape": (0.25, 0.58), "entry": (0.58, 0.72), "commit": (0.72, 0.86), "final": (0.86, 1.00)},
        "relative_late": {"shape": (0.35, 0.68), "entry": (0.68, 0.80), "commit": (0.80, 0.92), "final": (0.92, 1.00)},
        "relative_wide_entry": {"shape": (0.25, 0.60), "entry": (0.60, 0.80), "commit": (0.80, 0.92), "final": (0.92, 1.00)},
    }
...
def transition_oof(src, dst, groups, alpha=1.0):
    ...
    return {"cbit": cbit, "conditional_prob": conditional_prob, "mean_cbit": float(np.mean(cbit))}


def topk_jaccard(ids_a, ids_b):
...
def topk_weighted_jaccard(ids_a, probs_a, ids_b, probs_b):
    da = {int(i): float(p) for i, p in zip(ids_a, probs_a)}
    db = {int(i): float(p) for i, p in zip(ids_b, probs_b)}
    keys = set(da) | set(db)
    if not keys:
        return 1.0
    num = sum(min(da.get(k, 0.0), db.get(k, 0.0)) for k in keys)
    den = sum(max(da.get(k, 0.0), db.get(k, 0.0)) for k in keys) + 1e-12
    return float(num / den)


def compute_independent_tq(pair_local, delta, windows):
    """
    Independent TQ proxies do NOT use Cbit as the target label.
    They use TopK flow, family consistency and answer-adjacent rank quality.
    """
    te = delta["topk_entropy_delta"]
    ts = delta["topk_spread_delta"]
    tc = delta["topk_center_delta"]
    ids = delta["topk_ids_cond"]
    probs = delta["topk_probs_cond"]

    n, L = te.shape
    entry_w = windows["entry"]
    commit_w = windows["commit"]
    final_w = windows["final"]
    late_w = sorted(set(entry_w + commit_w + final_w))

    # 1. Flow smoothness: inverse std of center movement magnitudes in late window.
    center_norm = tc / (np.linalg.norm(tc, axis=-1, keepdims=True) + 1e-12)
    if len(late_w) >= 2:
        late_centers = center_norm[:, late_w, :]
        step = np.linalg.norm(np.diff(late_centers, axis=1), axis=-1)
        flow_variation = step.std(axis=1)
        flow_mean = step.mean(axis=1)
    else:
        flow_variation = np.zeros(n)
        flow_mean = np.zeros(n)
    topk_flow_smoothness = -flow_variation

    # 2. Entropy stabilization: entry entropy - final entropy.
    entry_entropy = scalar_window(te, entry_w)
    final_entropy = scalar_window(te, final_w)
    entropy_stabilization = entry_entropy - final_entropy

    # 3. Spread stabilization: entry spread - final spread.
    entry_spread = scalar_window(ts, entry_w)
    final_spread = scalar_window(ts, final_w)
    spread_stabilization = entry_spread - final_spread

    # 4. Commit-final TopK set stability.
    commit_idx = commit_w[-1]
    final_idx = final_w[-1]
    jf = []
    wjf = []
    for i in range(n):
        jf.append(topk_jaccard(ids[i, commit_idx], ids[i, final_idx]))
        wjf.append(topk_weighted_jaccard(ids[i, commit_idx], probs[i, commit_idx], ids[i, final_idx], probs[i, final_idx]))
    commit_final_jaccard = np.asarray(jf)
    commit_final_weighted_jaccard = np.asarray(wjf)

    # 5. Within-task-family final-family consistency as sample-level proxy.
    # A sample gets the consistency of its task_family's dominant final family.
    final_family = pair_local["final_family"].values.astype(int)
    fam = pair_local["task_family_id"].values.astype(int)
    consistency_vals = np.zeros(n)
    for f in np.unique(fam):
        mask = fam == f
        counts = np.bincount(final_family[mask], minlength=int(final_family.max()) + 1)
        consistency_vals[mask] = counts.max() / max(1, counts.sum())

    # 6. answer-adjacent rank quality.
    rank_quality = pair_local["rank_quality"].values.astype(float)

    out = pair_local.copy()
    out["tq_flow_smoothness"] = topk_flow_smoothness
    out["tq_entropy_stabilization"] = entropy_stabilization
    out["tq_spread_stabilization"] = spread_stabilization
    out["tq_commit_final_jaccard"] = commit_final_jaccard
    out["tq_commit_final_weighted_jaccard"] = commit_final_weighted_jaccard
    out["tq_family_consistency"] = consistency_vals
    out["tq_answer_rank_quality"] = rank_quality

    independent_cols = [
        "tq_flow_smoothness",
        "tq_entropy_stabilization",
        "tq_spread_stabilization",
        "tq_commit_final_jaccard",
        "tq_commit_final_weighted_jaccard",
        "tq_family_consistency",
        "tq_answer_rank_quality",
    ]

    zsum = np.zeros(n)
    for col in independent_cols:
        zsum += zscore(out[col].values)
    out["tq_independent_composite"] = zsum / len(independent_cols)
    out["tq_independent_high"] = (out["tq_independent_composite"] >= np.median(out["tq_independent_composite"].values)).astype(int)
    return sanitize_dataframe_numeric(out, "independent_tq"), independent_cols
...
def reg_eval(X, y, groups):
    ...
    return {"corr": safe_corr(pred, y), "spearman": spr, "r2": float(r2_score(y, pred)), "rmse": float(math.sqrt(mean_squared_error(y, pred)))}
...
def clf_eval(X, y, groups):
    ...
    cv = GroupKFold(n_splits=min(5, len(np.unique(groups))))
    clf = make_pipeline(StandardScaler(with_mean=True, with_std=True), LogisticRegression(max_iter=1000, solver="lbfgs", class_weight="balanced", C=1.0))
    prob = cross_val_predict(clf, X, y, cv=cv.split(X, y, groups), method="predict_proba")[:, 1]
    pred = (prob >= 0.5).astype(int)
    return {"acc": float(accuracy_score(y, pred)), "macro_f1": float(f1_score(y, pred, average="macro")), "auc": float(roc_auc_score(y, prob))}


def evaluate_independent_tq(pair_local, groups, independent_cols):
    cbit_cols = ["cbit_shape_entry", "cbit_entry_commit", "cbit_commit_final", "stagewise_cbit", "transition_stability", "effective_cbit_proxy"]
    direct_cols = ["cbit_shape_final_direct", "cbit_shape_commit_direct"]
    all_tq_cols = independent_cols + ["tq_independent_composite"]

    y_tq = pair_local["tq_independent_composite"].values.astype(float)
    y_tq_high = pair_local["tq_independent_high"].values.astype(int)
    y_margin = pair_local["answer_margin"].values.astype(float)
    y_correct = pair_local["answer_correct"].values.astype(int)

    result = {
        "cbit_to_independent_tq": {
        ...
        "independent_tq_to_answer_margin": reg_eval(pair_local[all_tq_cols].values, y_margin, groups),
        "independent_tq_to_answer_correct": clf_eval(pair_local[all_tq_cols].values, y_correct, groups),
        ...
        "cbit_plus_independent_tq_to_answer_margin": reg_eval(pair_local[cbit_cols + all_tq_cols].values, y_margin, groups),
        "cbit_plus_independent_tq_to_answer_correct": clf_eval(pair_local[cbit_cols + all_tq_cols].values, y_correct, groups),
    }

    cbit_r2 = result["cbit_to_independent_tq"]["regression"]["r2"]
    direct_r2 = result["direct_to_independent_tq"]["regression"]["r2"]
    cbit_auc = result["cbit_to_independent_tq"]["classification"]["auc"]
    direct_auc = result["direct_to_independent_tq"]["classification"]["auc"]

    tq_ans_r2 = result["independent_tq_to_answer_margin"]["r2"]
    tq_ans_auc = result["independent_tq_to_answer_correct"]["auc"]
    cbit_ans_r2 = result["cbit_to_answer_margin"]["r2"]
    cbit_ans_auc = result["cbit_to_answer_correct"]["auc"]
    plus_ans_r2 = result["cbit_plus_independent_tq_to_answer_margin"]["r2"]
    plus_ans_auc = result["cbit_plus_independent_tq_to_answer_correct"]["auc"]

    flags = {
        "cbit_predicts_independent_tq": bool((cbit_r2 > 0.05) or (cbit_auc is not None and cbit_auc > 0.62)),
        "stagewise_beats_direct_for_independent_tq": bool((cbit_r2 > direct_r2 + 0.03) or (cbit_auc is not None and direct_auc is not None and cbit_auc > direct_auc + 0.03)),
        "independent_tq_predicts_answer": bool((tq_ans_r2 > 0.03) or (tq_ans_auc is not None and tq_ans_auc > 0.58)),
        "independent_tq_improves_answer_model": bool((plus_ans_r2 > cbit_ans_r2 + 0.02) or (plus_ans_auc is not None and cbit_ans_auc is not None and plus_ans_auc > cbit_ans_auc + 0.03)),
    }

    summary = {
        "cbit_to_independent_tq_r2": cbit_r2,
        "cbit_to_independent_tq_auc": cbit_auc,
        "direct_to_independent_tq_r2": direct_r2,
        "direct_to_independent_tq_auc": direct_auc,
        "independent_tq_to_answer_margin_r2": tq_ans_r2,
        "independent_tq_to_answer_correct_auc": tq_ans_auc,
        "cbit_to_answer_margin_r2": cbit_ans_r2,
        "cbit_to_answer_correct_auc": cbit_ans_auc,
        "cbit_plus_tq_to_answer_margin_r2": plus_ans_r2,
        "cbit_plus_tq_to_answer_correct_auc": plus_ans_auc,
        "mean_tq_independent_composite": float(pair_local["tq_independent_composite"].mean()),
        "std_tq_independent_composite": float(pair_local["tq_independent_composite"].std()),
    }
...
def score_window_profile(delta, pair_df, quality, windows, args, profile_name):
    ...
    pair_local, independent_cols = compute_independent_tq(pair_local, delta, windows)
    pair_local = sanitize_dataframe_numeric(pair_local, f"{profile_name}.pair_local_final")

    evals, flags, summary = evaluate_independent_tq(pair_local, groups, independent_cols)

    sel_score = 0.0
    sel_score += max(0.0, summary["cbit_to_independent_tq_r2"]) * 3.0
    if summary["cbit_to_independent_tq_auc"] is not None:
        sel_score += max(0.0, summary["cbit_to_independent_tq_auc"] - 0.5) * 3.0
    if summary["independent_tq_to_answer_correct_auc"] is not None:
        sel_score += max(0.0, summary["independent_tq_to_answer_correct_auc"] - 0.5) * 2.0
    sel_score += max(0.0, summary["independent_tq_to_answer_margin_r2"]) * 2.0
    sel_score += 0.5 if flags["stagewise_beats_direct_for_independent_tq"] else 0.0
...
def run_one_model(model_key, args, prompt_df, pair_df, model_out_dir):
    ...
    feats = extract_hidden_topk_sets(tokenizer, model, prompt_df, args.device, args.max_len, args.topk, batch_size=1)
    delta = build_delta_features(feats, pair_df)

    quality = score_candidates(tokenizer, model, pair_df, prompt_df, args.device, args.max_len, score_mode=args.score_mode)
    quality.to_csv(model_out_dir / f"psa4e_{model_key}_answer_quality.csv", index=False, encoding="utf-8-sig")
    ...
    pair_best.to_csv(model_out_dir / f"psa4e_{model_key}_pairs_with_independent_tq.csv", index=False, encoding="utf-8-sig")
    ...
    for r in profile_results:
        ...
        profile_table.append({
            ...
            "cbit_to_independent_tq_r2": sm["cbit_to_independent_tq_r2"],
            "cbit_to_independent_tq_auc": sm["cbit_to_independent_tq_auc"],
            "direct_to_independent_tq_r2": sm["direct_to_independent_tq_r2"],
            "direct_to_independent_tq_auc": sm["direct_to_independent_tq_auc"],
            "independent_tq_to_answer_correct_auc": sm["independent_tq_to_answer_correct_auc"],
            "independent_tq_to_answer_margin_r2": sm["independent_tq_to_answer_margin_r2"],
            ...
            "cbit_predicts_independent_tq": pf["cbit_predicts_independent_tq"],
            "stagewise_beats_direct_independent_tq": pf["stagewise_beats_direct_for_independent_tq"],
            "independent_tq_predicts_answer": pf["independent_tq_predicts_answer"],
            "independent_tq_improves_answer_model": pf["independent_tq_improves_answer_model"],
    ...
    pd.DataFrame(profile_table).to_csv(model_out_dir / f"psa4e_{model_key}_profile_scan.csv", index=False, encoding="utf-8-sig")

    pf = best["pass_flags"]
    if pf["cbit_predicts_independent_tq"] and pf["stagewise_beats_direct_for_independent_tq"] and pf["independent_tq_predicts_answer"]:
        verdict = "PASS_PSA4E_CBIT_TO_INDEPENDENT_TQ_TO_ANSWER"
    elif pf["cbit_predicts_independent_tq"] and pf["stagewise_beats_direct_for_independent_tq"]:
        verdict = "PARTIAL_PSA4E_CBIT_TO_INDEPENDENT_TQ"
    elif pf["cbit_predicts_independent_tq"]:
        verdict = "PARTIAL_PSA4E_CBIT_INDEPENDENT_TQ_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4E"
    ...
    summary = {
        ...
        "pass_flags": pf,
        "summary_metrics": {**best["summary_metrics"], **best["transition_summary"]},
        "mapping_results": best["mapping_results"],
        "independent_tq_columns": best["independent_cols"],
        "windows": best["windows"],
        "all_profile_results": profile_table,
        "model_path": MODEL_PATHS[model_key],
    }

    with open(model_out_dir / f"psa4e_{model_key}_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    with open(model_out_dir / f"psa4e_{model_key}_verdict.txt", "w", encoding="utf-8") as f:
...
def aggregate_results(model_summaries):
    ...
    for s in model_summaries:
        ...
        rows.append({
            ...
            "cbit_predicts_independent_tq": s["pass_flags"]["cbit_predicts_independent_tq"],
            "stagewise_beats_direct_independent_tq": s["pass_flags"]["stagewise_beats_direct_for_independent_tq"],
            "independent_tq_predicts_answer": s["pass_flags"]["independent_tq_predicts_answer"],
            "independent_tq_improves_answer_model": s["pass_flags"]["independent_tq_improves_answer_model"],
            "cbit_to_independent_tq_r2": m["cbit_to_independent_tq_r2"],
            "cbit_to_independent_tq_auc": m["cbit_to_independent_tq_auc"],
            "direct_to_independent_tq_r2": m["direct_to_independent_tq_r2"],
            "direct_to_independent_tq_auc": m["direct_to_independent_tq_auc"],
            "independent_tq_to_answer_correct_auc": m["independent_tq_to_answer_correct_auc"],
            "independent_tq_to_answer_margin_r2": m["independent_tq_to_answer_margin_r2"],
    ...
    n = len(df)
    n_cbit_tq = int(df["cbit_predicts_independent_tq"].sum())
    n_stage_beats = int(df["stagewise_beats_direct_independent_tq"].sum())
    n_tq_answer = int(df["independent_tq_predicts_answer"].sum())
    ...
    if n_cbit_tq >= max(1, n - 1) and n_stage_beats >= max(1, n - 1) and n_tq_answer >= max(1, n - 1):
        verdict = "PASS_PSA4E_CROSSMODEL_INDEPENDENT_TQ_CHAIN"
    elif n_cbit_tq >= max(1, n - 1) and n_stage_beats >= max(1, n - 1):
        verdict = "PARTIAL_PSA4E_CROSSMODEL_CBIT_TO_INDEPENDENT_TQ"
    elif n_cbit_tq >= max(1, n - 1):
        verdict = "PARTIAL_PSA4E_CROSSMODEL_CBIT_INDEPENDENT_TQ_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4E_CROSSMODEL"

    return df, {
        "verdict": verdict,
        "n_models": int(n),
        "n_pass_or_partial": n_pass_or_partial,
        "n_cbit_predicts_independent_tq": n_cbit_tq,
        "n_stagewise_beats_direct_for_independent_tq": n_stage_beats,
        "n_independent_tq_predicts_answer": n_tq_answer,
        "models": rows,
        "interpretation": (
            "PSA-4E tests whether Cbit predicts trajectory-quality proxies not directly constructed from Cbit. "
            "A positive result upgrades PSA-4D from coordinate closure to independent trajectory-quality evidence."
        ),
        "terminology": "Cbit / independent trajectory quality / answer quality / stagewise transition; no basin terminology",
    }
...
def main():
    ...
    prompt_df.to_csv(out_dir / "psa4e_prompts.csv", index=False, encoding="utf-8-sig")
    pair_df.to_csv(out_dir / "psa4e_pairs.csv", index=False, encoding="utf-8-sig")
    ...
    agg_df.to_csv(out_dir / "psa4e_crossmodel_summary_table.csv", index=False, encoding="utf-8-sig")

    agg["config"] = vars(args)
    with open(out_dir / "psa4e_crossmodel_summary.json", "w", encoding="utf-8") as f:
        json.dump(agg, f, ensure_ascii=False, indent=2)
    with open(out_dir / "psa4e_crossmodel_verdict.txt", "w", encoding="utf-8") as f:
