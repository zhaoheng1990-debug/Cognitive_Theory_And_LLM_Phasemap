# -*- coding: utf-8 -*-
r"""
GPT_212_DA_ASA_1C_override_multistage_attractor_steering.py

DA-ASA-1C: Override-Focused Multi-Stage Attractor Steering

背景：
DA-ASA-1B 显示：
- closure_exception / closure_negation 基本可被 condition-aware to_update 拉回；
- closure_override 可显著推动，但 clean-rate 只到约 0.5；
- hallucination_like 需要单独控制器；
- condition-aware 明显优于 fixed policy。

本实验专攻 closure_override：

核心假设：
    closure_override 是强 override attractor。
    单段 to_update 可以把它推离 wrong attractor，但不足以完全进入 clean basin。
    需要 multi-stage steering：

        Stage 1: L15-L17 to_update
                 先从 override attractor 推向 update / replacement side

        Stage 2: L18-L19 to_stable 或 suppress_wrong / correction
                 再把轨迹推入 clean basin 或降低 wrong attractor inertia

对照：
    no_intervention
    single_to_update_a020
    single_to_update_a030
    single_to_stable_a030
    multistage_update_then_stable
    multistage_update_then_suppress
    multistage_update_then_update
    multistage_soft_update_then_stable
    override_only_multistage_policy

输出：
    da_asa1c_outputs/
        da_asa1c_generated_prompts.csv
        da_asa1c_baseline_scores.csv
        da_asa1c_policy_results.csv
        da_asa1c_policy_summary.csv
        da_asa1c_condition_summary.csv
        da_asa1c_verdict_summary.csv
"""
...
CONFIG = {
    ...
    "OUTPUT_DIR": r"da_asa1c_outputs",

    "COLLECT_LAYERS": [15, 16, 17, 18, 19],
    "EARLY_LAYERS": [15, 16, 17],
    "LATE_LAYERS": [18, 19],
    "ALL_STEER_LAYERS": [15, 16, 17, 18, 19],
    ...
    # Policy definitions:
    # stage list of (layer_group, mode, alpha), layer_group in early/late/all
    # apply_to: "*" means all samples; or specific conditions.
    "POLICIES": {
        "no_intervention": {
            "*": []
        },
        ...
        # main multistage policies applied to all samples, to test raw effect and side effects
        "multi_update020_then_stable010_all": {
            "*": [("early", "to_update", 0.20), ("late", "to_stable", 0.10)]
        },
        "multi_update020_then_stable015_all": {
            "*": [("early", "to_update", 0.20), ("late", "to_stable", 0.15)]
        },
        "multi_update025_then_stable015_all": {
            "*": [("early", "to_update", 0.25), ("late", "to_stable", 0.15)]
        },
        "multi_update020_then_suppress015_all": {
            "*": [("early", "to_update", 0.20), ("late", "suppress_override", 0.15)]
        },
        "multi_update020_then_update010_all": {
            "*": [("early", "to_update", 0.20), ("late", "to_update", 0.10)]
        },

        # condition-aware: only intervene override, leave non-target untouched
        "override_only_update020_then_stable010": {
            "closure_override": [("early", "to_update", 0.20), ("late", "to_stable", 0.10)],
            "*": []
        },
        "override_only_update025_then_stable015": {
            "closure_override": [("early", "to_update", 0.25), ("late", "to_stable", 0.15)],
            "*": []
        },
        "override_only_update020_then_suppress015": {
            "closure_override": [("early", "to_update", 0.20), ("late", "suppress_override", 0.15)],
            "*": []
        },

        # broader condition-aware from 1B + special override multistage
        "condition_aware_1b_plus_override_multistage": {
...
def build_steering_vectors(df, hidden_np, out_dir):
    ...
    override_idx = np.where(np.isin(cond, CONFIG["OVERRIDE_ATTRACTOR_CONDITIONS"]))[0]
    ...
    vectors = {"to_stable": {}, "to_update": {}, "suppress_override": {}}
    rows = []
    ...
    pd.DataFrame(rows).to_csv(out_dir / "da_asa1c_steering_vector_summary.csv", index=False, encoding="utf-8-sig")
    return vectors
...
def layers_for_group(group_name: str):
    if group_name == "early":
        return set(CONFIG["EARLY_LAYERS"])
    if group_name == "late":
        return set(CONFIG["LATE_LAYERS"])
    if group_name == "all":
...
def run_policy_eval(model, tokenizer, df, vectors, out_dir):
    ...
    for policy_name in CONFIG["POLICIES"].keys():
        ...
        for start in range(0, len(df), CONFIG["BATCH_SIZE"]):
            ...
            def make_mixed_hook(layer_idx):
                def hook(module, inputs, output):
                    ...
                    for bi in range(h2.shape[0]):
                        ...
                        actions = policy_action(policy_name, cond)
                        if not actions:
                            continue

                        for layer_group, mode, alpha in actions:
                            if alpha == 0.0 or mode == "none":
                                continue
                            if layer_idx not in layers_for_group(layer_group):
                                continue
            ...
            for bi in range(input_ids.shape[0]):
                ...
                fl = logits[bi, pos, :].float()
                R = float(fl[c_id].item() - fl[e_id].item())
                actions = policy_action(policy_name, batch.loc[bi, "condition"])
    ...
    res.to_csv(out_dir / "da_asa1c_policy_results.csv", index=False, encoding="utf-8-sig")
    return res


def summarize(results, baseline, out_dir):
    base = baseline[["id", "baseline_R_final", "baseline_pred_clean"]].copy()
    df = results.merge(base, on="id", how="left")
    df["delta_R"] = df["R_final"] - df["baseline_R_final"]

    target = set(CONFIG["TARGET_CONDITIONS"])
    second = set(CONFIG["SECONDARY_TARGET_CONDITIONS"])
    halluc = set(CONFIG["HALLUCINATION_CONDITIONS"])
    ...
    summary.to_csv(out_dir / "da_asa1c_policy_summary.csv", index=False, encoding="utf-8-sig")
    ...
    cond_summary.to_csv(out_dir / "da_asa1c_condition_summary.csv", index=False, encoding="utf-8-sig")

    verdict_rows = []
    best_margin = summary.iloc[0]
    best_clean = summary.sort_values("override_specificity_vs_nontarget_clean_gain", ascending=False).iloc[0]

    fixed = summary[summary["policy"].str.startswith("single_")]
    multi = summary[summary["policy"].str.contains("multi|override_only|condition_aware", regex=True)]
    if len(fixed) and len(multi):
        best_fixed = fixed.sort_values("override_specificity_vs_nontarget_delta_R", ascending=False).iloc[0]
        best_multi = multi.sort_values("override_specificity_vs_nontarget_delta_R", ascending=False).iloc[0]
        verdict_rows.append({
            "claim": "Best multistage policy beats best single-stage policy on override margin specificity.",
            "best_multi_policy": best_multi["policy"],
            "best_single_policy": best_fixed["policy"],
            "multi_specificity_delta_R": float(best_multi["override_specificity_vs_nontarget_delta_R"]),
            "single_specificity_delta_R": float(best_fixed["override_specificity_vs_nontarget_delta_R"]),
            "gain": float(best_multi["override_specificity_vs_nontarget_delta_R"] - best_fixed["override_specificity_vs_nontarget_delta_R"]),
            "verdict": "PASS_MULTI_BEATS_SINGLE" if best_multi["override_specificity_vs_nontarget_delta_R"] > best_fixed["override_specificity_vs_nontarget_delta_R"] else "FAIL_MULTI_NOT_BEAT_SINGLE",
        })

    verdict_rows.append({
        "claim": "Best policy improves override clean-rate without harming non-target.",
        "best_policy": best_clean["policy"],
        "override_clean_rate_gain": float(best_clean["clean_rate_gain_override"]),
        "nontarget_clean_rate_gain": float(best_clean["clean_rate_gain_nontarget"]),
        "specificity_clean_gain": float(best_clean["override_specificity_vs_nontarget_clean_gain"]),
        "verdict": "PASS_OVERRIDE_CLEAN_GAIN" if best_clean["override_specificity_vs_nontarget_clean_gain"] > 0 else "FAIL_OVERRIDE_CLEAN_GAIN",
    })

    verdict = pd.DataFrame(verdict_rows)
    verdict.to_csv(out_dir / "da_asa1c_verdict_summary.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    with open(out_dir / "da_asa1c_config.json", "w", encoding="utf-8") as f:
        json.dump(CONFIG, f, ensure_ascii=False, indent=2)

    df = build_prompt_dataset()
    df.to_csv(out_dir / "da_asa1c_generated_prompts.csv", index=False, encoding="utf-8-sig")

    model, tokenizer = load_model_and_tokenizer(CONFIG["MODEL_PATH"], CONFIG["DEVICE"], CONFIG["DTYPE"])

    tok = tokenize_candidates(df, tokenizer)
    tok.to_csv(out_dir / "da_asa1c_candidate_tokenization.csv", index=False, encoding="utf-8-sig")
    ...
    baseline.to_csv(out_dir / "da_asa1c_baseline_scores.csv", index=False, encoding="utf-8-sig")

    vectors = build_steering_vectors(df, hidden, out_dir)
    ...
    for name in [
        "da_asa1c_generated_prompts.csv",
        "da_asa1c_candidate_tokenization.csv",
        "da_asa1c_baseline_scores.csv",
        "da_asa1c_steering_vector_summary.csv",
        "da_asa1c_policy_results.csv",
        "da_asa1c_policy_summary.csv",
        "da_asa1c_condition_summary.csv",
        "da_asa1c_verdict_summary.csv",
