...
def _range_tuple(start: int, end_inclusive: int, num_layers: int) -> Tuple[int, ...]:
    """Safe inclusive layer range clipped to [0, num_layers-1]."""
    start = max(0, min(num_layers - 1, int(start)))
    end_inclusive = max(start, min(num_layers - 1, int(end_inclusive)))
    return tuple(range(start, end_inclusive + 1))


def _frac_range(a: float, b: float, num_layers: int) -> Tuple[int, ...]:
    """Inclusive fractional layer window."""
    l0 = int(math.floor(num_layers * a))
    l1 = int(math.floor(num_layers * b))
    return _range_tuple(l0, l1, num_layers)


def apply_model_specific_layer_config(cfg: Config, model) -> Config:
    """
    ASA-1 must not reuse Qwen absolute layer windows for every model.

    CM-4D showed model-specific initialization windows:
      Qwen : mechanism/downstream peak L5-L6; topology stabilizes around L9-L11.
      Llama: topology/downstream very early L0-L2; mechanism L2-L5.
      Gemma: topology/downstream L0-L1; mechanism L7-L8.

    This function rewrites cfg.proto_layers, cfg.decision_layers, and
    cfg.steering_windows after the model is loaded and num_layers is known.
    """
    num_layers = len(get_layers(model))
    key = str(cfg.model_key).lower().strip()

    if key == "qwen":
        # Qwen2.5-1.5B-Instruct has 28 decoder layers in our setup.
        cfg.proto_layers = _range_tuple(0, min(19, num_layers - 1), num_layers)
        cfg.decision_layers = _range_tuple(min(20, num_layers - 1), min(25, num_layers - 1), num_layers)
        cfg.steering_windows = {
            "qwen_mech_L5_8": _range_tuple(5, 8, num_layers),
            "qwen_topoflow_L9_14": _range_tuple(9, 14, num_layers),
            "qwen_predecision_L15_19": _range_tuple(15, 19, num_layers),
        }

    elif key == "llama":
        # Llama-3.2-1B-Instruct: CM-4D suggested topology/downstream L0-L2,
        # mechanism L2-L5. Decision window uses relative late layers.
        cfg.proto_layers = _range_tuple(0, min(10, num_layers - 1), num_layers)
        cfg.decision_layers = _frac_range(0.70, 0.92, num_layers)
        cfg.steering_windows = {
            "llama_topology_L0_1": _range_tuple(0, 1, num_layers),
            "llama_downstream_seed_L0_2": _range_tuple(0, 2, num_layers),
            "llama_mechanism_L2_5": _range_tuple(2, 5, num_layers),
            "llama_mid_transport_L6_10": _range_tuple(6, min(10, num_layers - 1), num_layers),
        }

    elif key == "gemma":
        # Gemma-2-2B-it: CM-4D suggested topology/downstream L0-L1,
        # mechanism L7-L8, decision roughly L18-L23 for the 26-layer model.
        cfg.proto_layers = _range_tuple(0, min(14, num_layers - 1), num_layers)
        if num_layers >= 24:
            cfg.decision_layers = _range_tuple(18, min(23, num_layers - 1), num_layers)
        else:
            cfg.decision_layers = _frac_range(0.70, 0.92, num_layers)
        cfg.steering_windows = {
            "gemma_topology_L0_1": _range_tuple(0, 1, num_layers),
            "gemma_seed_L1_6": _range_tuple(1, min(6, num_layers - 1), num_layers),
            "gemma_mechanism_L7_8": _range_tuple(7, min(8, num_layers - 1), num_layers),
            "gemma_mid_transport_L9_14": _range_tuple(9, min(14, num_layers - 1), num_layers),
        }

    else:
        # Generic fallback for other decoder-only models.
        cfg.proto_layers = _range_tuple(0, max(1, int(math.floor(num_layers * 0.70)) - 1), num_layers)
        cfg.decision_layers = _frac_range(0.70, 0.92, num_layers)
        cfg.steering_windows = {
            "early_0_20pct": _frac_range(0.00, 0.20, num_layers),
            "mid_20_50pct": _frac_range(0.20, 0.50, num_layers),
            "predecision_50_70pct": _frac_range(0.50, 0.70, num_layers),
        }

    print("[LAYERS] Applied model-specific ASA-1 windows")
    print(f"[LAYERS] num_layers={num_layers}")
    print(f"[LAYERS] proto_layers={list(cfg.proto_layers)}")
    print(f"[LAYERS] decision_layers={list(cfg.decision_layers)}")
    print(f"[LAYERS] steering_windows={ {k: list(v) for k, v in cfg.steering_windows.items()} }")
    return cfg
...
def main(cfg: Config):
    ...
    cfg = apply_model_specific_layer_config(cfg, model)
