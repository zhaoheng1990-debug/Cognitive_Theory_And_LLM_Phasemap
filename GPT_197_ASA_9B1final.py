...
# ============================================================
# ASA-9B.1 Driver: Override-Aware Cross-Task Policy Audit
#
# Purpose:
#   ASA-9B-lite showed:
...
#
#   ASA-9B.1 adds override-aware submechanism labels while reusing the
#   ASA-8D engine:
#
#       stable
#       competition
...
# Outputs:
#   asa9b1_outputs/
#     asa9b1_cross_task_summary.csv
#     asa9b1_cross_task_summary.json
#     asa9b1_final_verdict.json
...
ROOT_SAVE_DIR = Path("./asa9b1_final_outputs")
...
def select_single_token_labels(tokenizer, save_dir):
    ...
    for lab in LABEL_CANDIDATES:
        ...
        if len(ids) == 1:
            chosen.append(lab)
    pd.DataFrame(rows).to_csv(Path(save_dir) / "asa9b1_label_tokenization_audit.csv", index=False, encoding="utf-8-sig")
    if len(chosen) < 12:
...
def build_override_aware_dataset(tokenizer, cfg, save_dir):
    ...
    df.to_csv(Path(save_dir) / "asa9b1_dataset.csv", index=False, encoding="utf-8-sig")
    return df
...
def patch_engine_for_six_mechanisms(engine):
    # The ASA-8D engine uses global MECH_TO_ID / ID_TO_MECH in feature/classifier helpers.
    # Patch them so predicted mechanism features become:
    ...
    # Rebuild feature columns to include all predicted mechanism probabilities / one-hot predictions.
    engine.POLICY_PRED_MECH_COLS = list(engine.MECH_FEATURE_COLS) + [
        "p_mech_stable",
        "p_mech_competition",
    ...
    # Robust predicted-mechanism attachment:
    # sklearn classifiers may not expose probability columns for classes absent
    # in a LOTFO train split. ASA-8D's policy feature list expects all columns.
    ...
    def _attach_predicted_mechanism_sixway(features, clf):
        ...
        return ensure_six_mech_columns(out)
...
def ensure_six_mech_columns(df):
    ...
    Some heldout splits may leave one or more six-way mechanism classes absent
    from policy-train. sklearn then returns only clf.classes_ probabilities,
    so ASA-8D's attach_predicted_mechanism may not create all expected columns.
    We fill absent classes with 0.0.
    """
    out = df.copy()
...
def make_lotfo_split_func(heldout_family):
    def lotfo_split(df, cfg, save_dir):
        ...
        split_df.to_csv(Path(save_dir) / "asa9b1_lotfo_split.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    print(f"[ASA-9B.1] Using ASA-8D engine: {engine_file}")
    ...
    for heldout in TASK_FAMILIES_TO_HOLDOUT:
        print("\n" + "=" * 88)
        print(f"[ASA-9B.1] Override-aware LOTFO heldout = {heldout}")
        print("=" * 88)
        ...
        with open(save_dir / "asa9b1_run_config.json", "w", encoding="utf-8") as f:
            json.dump({
                "model_key": MODEL_KEY,
        ...
        try:
            ...
            print(f"[ASA-9B.1][ERROR] heldout={heldout}: {repr(e)}")
            traceback.print_exc()
            row = {
                "heldout_task_family": heldout,
                "status": "error",
            ...
            all_rows.append(row)
            with open(save_dir / "asa9b1_error.json", "w", encoding="utf-8") as f:
                json.dump(row, f, indent=2, ensure_ascii=False, default=to_jsonable)

    summary_df = pd.DataFrame(all_rows)
    summary_df.to_csv(ROOT_SAVE_DIR / "asa9b1_cross_task_summary.csv", index=False, encoding="utf-8-sig")

    with open(ROOT_SAVE_DIR / "asa9b1_cross_task_summary.json", "w", encoding="utf-8") as f:
    ...
    final = {
        "audit": "ASA-9B.1 Override-Aware Cross-Task Policy Audit",
        "model": MODEL_KEY,
        "task_families": TASK_FAMILIES,
        "task_families_to_holdout": TASK_FAMILIES_TO_HOLDOUT,
        "mechanism_space": MECH_TO_ID_6,
        ...
        "n_shuffles": N_SHUFFLES,
        "summary_csv": "asa9b1_cross_task_summary.csv",
        "interpretation_rules": {
            "exception_improves": "Override-aware submechanisms repair the ASA-9B exception/override transfer failure.",
            "temporal_remains_pass": "Temporal update remains transferable under finer mechanism space.",
            "still_fails_exception": "Need richer rule-binding / evidence-binding features beyond labels.",
        },
    }

    with open(ROOT_SAVE_DIR / "asa9b1_final_verdict.json", "w", encoding="utf-8") as f:
