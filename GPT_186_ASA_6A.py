...
# ============================================================
# ASA-6A: Position Ridge Steering Specificity Audit
#
# Motivation:
#   ASA-5E failed the safe curvature direction audit, but revealed a strong
#   new candidate:
#
#       position_ridge specificity >> curvature / velocity / random
#
#   ASA-6A gives position_ridge the same level of scrutiny:
#     - matched shuffled-position ridge distribution
#     - random control
#     - answer positive control
#     - velocity_ridge / curvature_ridge controls
#     - orthogonal-to-answer position direction
#     - window ablation
#
# Core:
#   Learn hidden-position directions from:
#
#       H_l -> DeltaU
#
#   using only answerless target DeltaU constructed from decision-layer
#   trajectory PC1. No answer-token logit/rank/margin is used to construct
#   position directions. Answer token vectors are used only for the answer
#   positive control and answer-orthogonal control.
#
# Default:
#   Qwen, Q->S, position layers L15-L19,
#   decision layers L20-L25,
#   50 shuffled position-ridge controls.
#
# Outputs:
#   asa6a_outputs/
#     asa6a_config.json
#     asa6a_dataset.csv
#     asa6a_baseline_features.csv
#     asa6a_split.csv
#     asa6a_direction_audit.csv
#     asa6a_steering_results.csv
#     asa6a_specificity_summary.csv
#     asa6a_shuffle_specificity_significance.csv
#     asa6a_dose_response.csv
#     asa6a_layer_ablation_summary.csv
#     asa6a_verdict.csv
#     asa6a_summary.json
# ============================================================
...
class Config:
    ...
    # Candidate position steering layers.
    ...
    # DeltaU evaluation window.
    decision_layers: Tuple[int, ...] = tuple(range(20, 26))

    # Full-window and layer ablations.
    steering_windows: Dict[str, Tuple[int, ...]] = None

    alphas: Tuple[float, ...] = (0.0, 0.005, 0.01, 0.02, 0.05, 0.10, 0.20, 0.30)
...
def select_single_token_labels(tokenizer, save_dir):
    ...
    for lab in LABEL_CANDIDATES:
        ...
        if len(ids) == 1:
            chosen.append(lab)
    pd.DataFrame(rows).to_csv(save_dir / "asa6a_label_tokenization_audit.csv", index=False, encoding="utf-8-sig")
    if len(chosen) < 12:
...
def build_dataset(tokenizer, cfg, save_dir):
    ...
    df.to_csv(save_dir / "asa6a_dataset.csv", index=False, encoding="utf-8-sig")
    return df
...
def extract_baseline(model, tokenizer, df, cfg, save_dir):
    ...
    out.to_csv(save_dir / "asa6a_baseline_features.csv", index=False, encoding="utf-8-sig")
...
def make_split(df, cfg, save_dir):
    ...
    split_df.to_csv(save_dir / "asa6a_split.csv", index=False, encoding="utf-8-sig")
...
def fit_directions(base_df, h_by_layer, train_idx, cfg, save_dir):
    ...
    audits = []
    rng = np.random.default_rng(cfg.seed + 222)
    ...
    # Curvature a_l = H_{l+2}-2H_{l+1}+H_l -> DeltaU, hook at l+1.
    ...
    pd.DataFrame(audits).to_csv(save_dir / "asa6a_direction_audit.csv", index=False, encoding="utf-8-sig")
        ...
        for window in sorted(spec_df["window"].unique()):
            for alpha in cfg.shuffle_alphas:
                ...
                rows.append({
                    ...
                    "curvature_ridge_specificity": curv,
                    ...
                    "pos_minus_curvature": proto_spec - curv if np.isfinite(curv) else np.nan,
...
def main(cfg):
    ...
    with open(save_dir / "asa6a_config.json", "w", encoding="utf-8") as f:
        ...
        for window_name, window_layers_tuple in cfg.steering_windows.items():
            ...
            for alpha in cfg.alphas:
                ...
                if cfg.include_velocity_ridge:
                    # velocity hook layers are l+1, so overlap by hook layer.
                ...
                if cfg.include_curvature_ridge:
                    d = {l: directions["curvature_ridge"][l] for l in window_layers if l in directions["curvature_ridge"]}
                    control_maps["curvature_ridge"] = (d, list(d.keys()), "vector")
                ...
                if cfg.include_answer_control:
                    # dummy dmap; mode answer ignores actual direction values.
                    d = {l: directions["position_ridge"][l] for l in window_layers if l in directions["position_ridge"]}
    ...
    results_df.to_csv(save_dir / "asa6a_steering_results.csv", index=False, encoding="utf-8-sig")

    spec_df, merged = summarize_specificity(results_df, base_test, direction_targets)
    spec_df.to_csv(save_dir / "asa6a_specificity_summary.csv", index=False, encoding="utf-8-sig")
    merged.to_csv(save_dir / "asa6a_merged_shift_table.csv", index=False, encoding="utf-8-sig")

    dose_df = dose_response(spec_df)
    dose_df.to_csv(save_dir / "asa6a_dose_response.csv", index=False, encoding="utf-8-sig")

    sig_df = shuffle_significance(spec_df, cfg)
    sig_df.to_csv(save_dir / "asa6a_shuffle_specificity_significance.csv", index=False, encoding="utf-8-sig")

    verdict = sig_df[
        (np.isclose(sig_df["alpha"], max(cfg.shuffle_alphas))) &
        (sig_df["window"] == "L15_19_full")
    ].copy()
    verdict.to_csv(save_dir / "asa6a_verdict.csv", index=False, encoding="utf-8-sig")

    # Layer ablation: alpha max, position_ridge only.
    layer_ablation = spec_df[
        (spec_df["control"] == "position_ridge") &
        (np.isclose(spec_df["alpha"], max(cfg.shuffle_alphas)))
    ].sort_values(["direction", "specificity"], ascending=[True, False])
    layer_ablation.to_csv(save_dir / "asa6a_layer_ablation_summary.csv", index=False, encoding="utf-8-sig")

    summary = {
        "audit": "ASA-6A Position Ridge Steering Specificity Audit",
        "config": asdict(cfg),
        ...
        "key_outputs": {
            "steering_results": "asa6a_steering_results.csv",
            "specificity": "asa6a_specificity_summary.csv",
            "dose_response": "asa6a_dose_response.csv",
            "shuffle_specificity_significance": "asa6a_shuffle_specificity_significance.csv",
            "verdict": "asa6a_verdict.csv",
            "direction_audit": "asa6a_direction_audit.csv",
            "layer_ablation": "asa6a_layer_ablation_summary.csv",
        },
        "note": (
            "ASA-6A audits the strong position_ridge hint found in ASA-5E. "
            "It uses matched shuffled-position ridge controls, random, answer, velocity/curvature controls, "
            "orthogonal-to-answer position control, and window/layer ablations. "
            "Position directions are learned from H_l -> DeltaU, with no answer-token logit/rank/margin used to construct them."
        )
    }
    with open(save_dir / "asa6a_summary.json", "w", encoding="utf-8") as f:
