...
# ============================================================
# ASA-8C: Predicted-Mechanism Closed-Loop Control Audit
#
# Motivation:
#   ASA-8B showed a strong oracle-style learned gain policy:
#
#       learned_policy_combo specificity >> fixed same_combo
#
#   but it used true mechanism proxy:
#
#       mechanism_closure / mechanism_stable / mechanism_competition
#
#   ASA-8C removes that oracle leakage:
#
#       true mechanism labels are used only to train a mechanism classifier
#       on policy-train graphs.
#
#   At test time, the gain policy sees only:
#
#       DeltaU_hat, D_B_hat, velocity/inertia proxy, layer features,
#       predicted p_closure / p_stable / p_competition
#
#   It does NOT receive true mechanism labels.
#
# Main question:
#   Can predicted-mechanism closed-loop control approach oracle 8B and
#   outperform fixed same_combo?
#
# Controls:
#   none
#   operator_only_fixed
...
#   same_combo_fixed
#   learned_policy_predmech_combo
#   learned_policy_predmech_safe_combo
#   oracle_policy_combo          # diagnostic upper bound, not final claim
#   random_gain_combo
#   answer
#
# Outputs:
...
#     asa8c_config.json
#     asa8c_dataset.csv
#     asa8c_baseline_features.csv
#     asa8c_mechanism_classifier_audit.csv
#     asa8c_policy_train_grid_results.csv
#     asa8c_policy_train_grid_labels.csv
#     asa8c_policy_audit.csv
#     asa8c_predicted_mechanism_features_train.csv
#     asa8c_predicted_mechanism_features_test.csv
#     asa8c_learned_policy_predmech_actions.csv
#     asa8c_oracle_policy_actions.csv
#     asa8c_control_trace.csv
#     asa8c_gain_profile.csv
#     asa8c_steering_results.csv
#     asa8c_specificity_summary.csv
#     asa8c_verdict.csv
#     asa8c_summary.json
# ============================================================
...
class Config:
    ...
    mechanism_clf_type: str = "random_forest"  # "random_forest" or "logreg"
    ...
    include_oracle_policy_upper_bound: bool = True
...
def select_single_token_labels(tokenizer, save_dir):
    ...
    for lab in LABEL_CANDIDATES:
        ...
        if len(ids) == 1:
            chosen.append(lab)
    pd.DataFrame(rows).to_csv(save_dir / "asa8c_label_tokenization_audit.csv", index=False, encoding="utf-8-sig")
    if len(chosen) < 12:
...
def build_dataset(tokenizer, cfg, save_dir):
    ...
    df.to_csv(save_dir / "asa8c_dataset.csv", index=False, encoding="utf-8-sig")
    return df
...
def extract_baseline(model, tokenizer, df, cfg, save_dir):
    ...
    out.to_csv(save_dir / "asa8c_baseline_features.csv", index=False, encoding="utf-8-sig")
...
def make_split(df, cfg, save_dir):
    ...
    split_df.to_csv(save_dir / "asa8c_split.csv", index=False, encoding="utf-8-sig")
...
def fit_stable_operators(base_df, h_by_layer, train_idx, cfg, save_dir):
    ...
    pd.DataFrame(audits).to_csv(save_dir / "asa8c_operator_audit.csv", index=False, encoding="utf-8-sig")
    return operators
...
def fit_precursor_and_probes(base_df, h_by_layer, train_idx, cfg, save_dir):
    ...
    pd.DataFrame(audits).to_csv(save_dir / "asa8c_probe_audit.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{
        "layer": l,
    ...
    } for l in cfg.precursor_layers]).to_csv(save_dir / "asa8c_direction_audit.csv", index=False, encoding="utf-8-sig")
...
def base_state_features_for_samples(base_df, h_by_layer, probes, vel_stats, sample_idx, cfg, include_true_mech=False):
    rows = []
...
POLICY_ORACLE_COLS = POLICY_BASE_COLS + [
    "mechanism_stable", "mechanism_competition", "mechanism_closure",
]
...
def fit_mechanism_classifier(train_features, cfg, save_dir):
    X = train_features[MECH_FEATURE_COLS].values.astype(np.float32)
    y = train_features["mechanism_id"].values.astype(int)
    groups = train_features["graph_id"].values

    if cfg.mechanism_clf_type == "logreg":
        clf = Pipeline([
    ...
    if hasattr(clf, "predict_proba"):
        ...
        prob = np.zeros((len(y), 3), dtype=np.float32)

    # GroupKFold CV diagnostic.
    n_splits = min(5, len(np.unique(groups)))
    gkf = GroupKFold(n_splits=n_splits)
    cv_pred = np.zeros_like(y)
    cv_prob = np.zeros((len(y), 3), dtype=np.float32)
    for tr, te in gkf.split(X, y, groups):
        if cfg.mechanism_clf_type == "logreg":
            ...
            c = RandomForestClassifier(
                ...
                min_samples_leaf=6,
                class_weight="balanced_subsample",
                random_state=cfg.seed + 17,
            )
        c.fit(X[tr], y[tr])
        cv_pred[te] = c.predict(X[te])
        cv_prob[te] = c.predict_proba(X[te])

    audit = {
        "train_accuracy": float(accuracy_score(y, pred)),
        "train_macro_f1": float(f1_score(y, pred, average="macro")),
        "groupcv_accuracy": float(accuracy_score(y, cv_pred)),
        "groupcv_macro_f1": float(f1_score(y, cv_pred, average="macro")),
        "groupcv_logloss": float(log_loss(y, cv_prob, labels=[0,1,2])),
        "n_rows": int(len(y)),
        "class_counts": pd.Series(y).map(ID_TO_MECH).value_counts().to_dict(),
        "feature_cols": MECH_FEATURE_COLS,
    }
    pd.DataFrame([audit]).to_csv(save_dir / "asa8c_mechanism_classifier_audit.csv", index=False, encoding="utf-8-sig")
    with open(save_dir / "asa8c_mechanism_classifier_audit.json", "w", encoding="utf-8") as f:
...
def fit_gain_policy(train_features, grid_labels, feature_cols, cfg, save_dir, name="predmech"):
    ...
    pd.DataFrame([audit]).to_csv(save_dir / f"asa8c_policy_audit_{name}.csv", index=False, encoding="utf-8-sig")
    return clf, audit

def policy_predict_actions(clf, feature_rows, feature_cols, cfg, safe=False):
    ...
    def hook_factory(l):
        def hook(module, inputs, output):
            ...
            if control_mode == "operator_only_fixed":
                ...
                alpha[:] = cfg.fixed_alpha
                beta[:] = cfg.fixed_beta
            elif control_mode in ("learned_policy_predmech_combo", "learned_policy_predmech_safe_combo", "oracle_policy_combo"):
...
def run_policy_train_grid(model, tokenizer, policy_train_df, cfg, pca_decision, clean_anchor_R,
    ...
    for a, b in actions:
        ...
        out, _ = forward_eval(
            ...
            stable_ops=stable_ops,
            precursor_dirs=precursor_dirs,
            control_mode="learned_policy_predmech_combo",
    ...
    grid_df.to_csv(save_dir / "asa8c_policy_train_grid_results.csv", index=False, encoding="utf-8-sig")
    ...
    label_df.to_csv(save_dir / "asa8c_policy_train_grid_labels.csv", index=False, encoding="utf-8-sig")
...
def main(cfg):
    ...
    with open(save_dir / "asa8c_config.json", "w", encoding="utf-8") as f:
    ...
    # Base state features.
    train_base_features = base_state_features_for_samples(base_df, h_by_layer, probes, vel_stats, train_idx, cfg, include_true_mech=True)
    test_base_features = base_state_features_for_samples(base_df, h_by_layer, probes, vel_stats, test_idx, cfg, include_true_mech=True)

    # Mechanism classifier on policy-train only.
    mech_clf, mech_audit = fit_mechanism_classifier(train_base_features, cfg, save_dir)

    train_pred_features = attach_predicted_mechanism(train_base_features, mech_clf)
    test_pred_features = attach_predicted_mechanism(test_base_features, mech_clf)
    train_pred_features.to_csv(save_dir / "asa8c_predicted_mechanism_features_train.csv", index=False, encoding="utf-8-sig")
    test_pred_features.to_csv(save_dir / "asa8c_predicted_mechanism_features_test.csv", index=False, encoding="utf-8-sig")

    # Run dose grid on policy-train.
    ...
    # Fit predicted-mechanism policy.
    pred_policy_clf, pred_policy_audit = fit_gain_policy(
        train_pred_features,
        grid_labels,
        POLICY_PRED_MECH_COLS,
        cfg,
        save_dir,
        name="predmech"
    )

    learned_pred_actions = policy_predict_actions(pred_policy_clf, test_pred_features, POLICY_PRED_MECH_COLS, cfg, safe=False)
    learned_pred_actions.to_csv(save_dir / "asa8c_learned_policy_predmech_actions.csv", index=False, encoding="utf-8-sig")
    learned_pred_safe_actions = policy_predict_actions(pred_policy_clf, test_pred_features, POLICY_PRED_MECH_COLS, cfg, safe=True)
    learned_pred_safe_actions.to_csv(save_dir / "asa8c_learned_policy_predmech_safe_actions.csv", index=False, encoding="utf-8-sig")

    oracle_actions = None
    oracle_policy_audit = None
    if cfg.include_oracle_policy_upper_bound:
        oracle_policy_clf, oracle_policy_audit = fit_gain_policy(
            train_base_features,
            grid_labels,
            POLICY_ORACLE_COLS,
            cfg,
        ...
        oracle_actions = policy_predict_actions(oracle_policy_clf, test_base_features, POLICY_ORACLE_COLS, cfg, safe=False)
        oracle_actions.to_csv(save_dir / "asa8c_oracle_policy_actions.csv", index=False, encoding="utf-8-sig")
    ...
    controls = [
        ...
        ("learned_policy_predmech_combo", learned_pred_actions),
        ("learned_policy_predmech_safe_combo", learned_pred_safe_actions),
        ("random_gain_combo", None),
    ]
    if cfg.include_oracle_policy_upper_bound and oracle_actions is not None:
        controls.append(("oracle_policy_combo", oracle_actions))
    ...
    results_df.to_csv(save_dir / "asa8c_steering_results.csv", index=False, encoding="utf-8-sig")

    spec_df, merged = summarize_specificity(results_df, base_test)
    spec_df.to_csv(save_dir / "asa8c_specificity_summary.csv", index=False, encoding="utf-8-sig")
    merged.to_csv(save_dir / "asa8c_merged_shift_table.csv", index=False, encoding="utf-8-sig")

    if len(traces):
        trace_df = pd.concat(traces, ignore_index=True)
    else:
        trace_df = pd.DataFrame()
    trace_df.to_csv(save_dir / "asa8c_control_trace.csv", index=False, encoding="utf-8-sig")

    gain_df = summarize_gain_trace(trace_df)
    gain_df.to_csv(save_dir / "asa8c_gain_profile.csv", index=False, encoding="utf-8-sig")
    ...
    pred = spec_df[spec_df["control"].isin(["learned_policy_predmech_combo", "learned_policy_predmech_safe_combo"])]
    best_pred = pred.sort_values("specificity", ascending=False).head(1).iloc[0].to_dict() if len(pred) else {}
    pred_gain_over_fixed = best_pred.get("specificity", np.nan) - fixed_spec if len(best_pred) else np.nan

    oracle = spec_df[spec_df["control"] == "oracle_policy_combo"]
    oracle_spec = float(oracle.iloc[0]["specificity"]) if len(oracle) else np.nan

    if len(best_pred) and pred_gain_over_fixed > 0.10:
        verdict = "PASS_PREDMECH_POLICY_IMPROVES_FIXED"
    elif len(best_pred) and pred_gain_over_fixed > 0.03:
        verdict = "PASS_LITE_PREDMECH_POLICY_HINT"
    ...
    verdict_df = pd.DataFrame([{
        "audit": "ASA-8C Predicted-Mechanism Closed-Loop Control Audit",
        "verdict": verdict,
        "best_control": best.get("control"),
        "best_specificity": best.get("specificity"),
        "fixed_combo_specificity": fixed_spec,
        "best_predmech_control": best_pred.get("control", None),
        "best_predmech_specificity": best_pred.get("specificity", np.nan),
        "predmech_gain_over_fixed": pred_gain_over_fixed,
        "oracle_policy_specificity": oracle_spec,
        "mech_groupcv_accuracy": mech_audit.get("groupcv_accuracy"),
        "mech_groupcv_macro_f1": mech_audit.get("groupcv_macro_f1"),
        "pred_policy_train_accuracy": pred_policy_audit.get("train_accuracy"),
        "pred_policy_train_macro_f1": pred_policy_audit.get("train_macro_f1"),
    }])
    verdict_df.to_csv(save_dir / "asa8c_verdict.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "verdict": verdict,
        "mechanism_classifier_audit": mech_audit,
        "pred_policy_audit": pred_policy_audit,
        "oracle_policy_audit": oracle_policy_audit,
        "best_specificity_row": best,
        "best_predmech_row": best_pred,
        "fixed_combo_specificity": fixed_spec,
        "predmech_gain_over_fixed": pred_gain_over_fixed,
        "oracle_policy_specificity": oracle_spec,
        "key_outputs": {
            "mechanism_classifier_audit": "asa8c_mechanism_classifier_audit.csv",
            "policy_train_grid_results": "asa8c_policy_train_grid_results.csv",
            "policy_train_grid_labels": "asa8c_policy_train_grid_labels.csv",
            "predicted_mechanism_features_train": "asa8c_predicted_mechanism_features_train.csv",
            "predicted_mechanism_features_test": "asa8c_predicted_mechanism_features_test.csv",
            "learned_policy_predmech_actions": "asa8c_learned_policy_predmech_actions.csv",
            "oracle_policy_actions": "asa8c_oracle_policy_actions.csv",
            "steering_results": "asa8c_steering_results.csv",
            "specificity": "asa8c_specificity_summary.csv",
            "control_trace": "asa8c_control_trace.csv",
            "gain_profile": "asa8c_gain_profile.csv",
            "verdict": "asa8c_verdict.csv",
        },
        "note": (
            "ASA-8C removes true mechanism labels from the deployed policy. "
            "Mechanism labels are used only for training a mechanism classifier on policy-train graphs. "
            "If predicted-mechanism policy beats fixed same_combo, run ASA-8D with matched shuffle/null controls."
        )
    }

    with open(save_dir / "asa8c_summary.json", "w", encoding="utf-8") as f:
