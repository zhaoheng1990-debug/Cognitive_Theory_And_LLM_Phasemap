# -*- coding: utf-8 -*-
# DA-ASA-2D.2c Predicted Policy Table Replay
# Fixed: fixed-policy replay no longer drops _fixed_action before merge.
# Default paths are relative to current working directory.
...
def infer_action_from_policy(policy_name):
    ...
    # Whole rule policies are not single reusable atomic actions for this canonical replay.
...
def build_outcome_table(pol):
    ...
    for action, priorities in CANONICAL_POLICY_PRIORITY.items():
        priority_norms = [p.lower() for p in priorities]
        candidates = pol[pol["_policy_raw_norm"].isin(priority_norms)].copy()
        if candidates.empty:
...
def merge_mode(pred, base, outcomes, mode_name, action_col):
    keep_cols = ["_key", "_graph_norm", "_condition_norm", "_true_policy_norm", "_pred_policy_norm"]
    if action_col not in pred.columns:
        raise KeyError(f"{mode_name}: requested action column not found: {action_col}. Columns={list(pred.columns)}")
    keep_cols.append(action_col)

    tmp = pred[keep_cols].copy()
    tmp["_requested_action"] = tmp[action_col].map(norm_policy_label)
    ...
    out_cols = ["_key", "_selected_action_class", "_selected_policy_raw"]
    if "R_final" in outcomes.columns:
        out_cols.append("R_final")
    if "pred_clean" in outcomes.columns:
        out_cols.append("pred_clean")
    if "C_clean" in outcomes.columns:
        out_cols.append("C_clean")
    if "E_conflict" in outcomes.columns:
        out_cols.append("E_conflict")

    outcome_view = outcomes[out_cols].copy()
    rename_map = {
        "_selected_action_class": "_requested_action",
        "R_final": "replay_R_final",
        "pred_clean": "replay_pred_clean",
        "C_clean": "replay_C_clean",
        "E_conflict": "replay_E_conflict",
    }
    outcome_view = outcome_view.rename(columns=rename_map)
    ...
    merged["missing_action_outcome"] = merged.get("replay_R_final", pd.Series(np.nan, index=merged.index)).isna() & merged.get("replay_pred_clean", pd.Series(np.nan, index=merged.index)).isna()
    merged["replay_pred_clean_num"] = clean_bool_series(merged.get("replay_pred_clean", pd.Series(np.nan, index=merged.index)))
    merged["replay_R_final_num"] = pd.to_numeric(merged.get("replay_R_final", pd.Series(np.nan, index=merged.index)), errors="coerce")
...
def summarize_mode(df, mode_name):
    ...
    target_gain = target_clean - target_base_clean if pd.notna(target_clean) and pd.notna(target_base_clean) else float("nan")
    ...
    return {
        ...
        "safety_adjusted_target_gain": target_gain - protected_harm if pd.notna(target_gain) else float("nan"),
    }
...
def main():
    ...
    ap.add_argument("--out-dir", default=str(root / "da_asa_2d2c_outputs"))
    args = ap.parse_args()
    ...
    pred_action_found = float(np.mean([(k, a) in action_pairs for k, a in zip(pred["_key"], pred["_pred_policy_norm"])]))
    oracle_action_found = float(np.mean([(k, a) in action_pairs for k, a in zip(pred["_key"], pred["_true_policy_norm"])]))
    ...
    replay.to_csv(out_dir / "da_asa2d2c_sample_replay.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(out_dir / "da_asa2d2c_replay_summary.csv", index=False, encoding="utf-8-sig")
    ...
    cond_summary.to_csv(out_dir / "da_asa2d2c_condition_summary.csv", index=False, encoding="utf-8-sig")
    ...
    verdict_obj = {
        "experiment": "DA-ASA-2D.2c Predicted Policy Table Replay",
        ...
        "caveats": [
            ...
            "Known-condition graph-heldout setting only; unseen-condition generalization is not tested here.",
        ],
        "outputs": {
            "sample_replay": str(out_dir / "da_asa2d2c_sample_replay.csv"),
            "replay_summary": str(out_dir / "da_asa2d2c_replay_summary.csv"),
            "condition_summary": str(out_dir / "da_asa2d2c_condition_summary.csv"),
            "verdict": str(out_dir / "da_asa2d2c_verdict.json"),
        },
    }

    with open(out_dir / "da_asa2d2c_verdict.json", "w", encoding="utf-8") as f:
