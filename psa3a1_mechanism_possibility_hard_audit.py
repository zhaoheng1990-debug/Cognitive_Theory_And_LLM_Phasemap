...
DEFAULT_OUT_DIR = r"C:\Users\ZH\Desktop\AGI\outputs\psa3a_hard_outputs"
...
SURFACES = [
    "Use the record table below and infer the active endpoint.",
    "Read the symbolic records and determine the endpoint allowed by the records.",
    "Inspect the three relation lines and state the endpoint that follows.",
    "Given the compact relation record, choose the endpoint licensed by the record.",
    "Analyze the relation mini-world and infer the resulting endpoint.",
    "Use only the records below to determine the active endpoint.",
]

QUESTION_FORMS = [
    "Return only the endpoint token.",
    "Answer with one endpoint token.",
    "Write the endpoint token only.",
    "Give only the resulting endpoint.",
]

def make_prompt(mech: str, entity_tuple: Tuple[str, str, str, str], surface: str, idx: int) -> str:
    """
    PSA-3A.1 hard prompt.

    Important design changes vs PSA-3A.0:
    - No mechanism names in the prompt.
    - No option list containing labels.
    - Same surface grammar across mechanism families.
    - The mechanism label is latent in the relation pattern.
    - Every prompt asks for an endpoint, not for the mechanism name.

    This prevents the layer-1 probe from solving the task through label-word leakage.
    """
    a, b, c, d = entity_tuple
    qform = QUESTION_FORMS[idx % len(QUESTION_FORMS)]

    # Use same lexical frame for all families.
    # Mechanism is encoded by relation topology / priority pattern, not explicit words.
    if mech == "stable":
        lines = [
            f"Line 1: {a} carries key {b}.",
            f"Line 2: key {b} maps to endpoint {c}.",
            f"Line 3: current table keeps endpoint {c}.",
        ]
    elif mech == "competition":
        lines = [
            f"Line 1: {a} carries key {b}.",
            f"Line 2: key {b} maps to endpoint {c}.",
            f"Line 3: parallel table maps key {b} to endpoint {d}.",
        ]
    elif mech == "closure":
        lines = [
            f"Line 1: {a} carries key {b}.",
            f"Line 2: key {b} maps to endpoint {c}.",
            f"Line 3: current table maps key {b} to endpoint {d}.",
        ]
    elif mech == "logic":
        lines = [
            f"Line 1: if {a} carries key {b}, then marker {c} is active.",
            f"Line 2: if marker {c} is active, endpoint {d} is active.",
            f"Line 3: {a} carries key {b}.",
        ]
    elif mech == "random":
        lines = [
            f"Line 1: {a} carries color {b}.",
            f"Line 2: token {c} names a weather fragment.",
            f"Line 3: endpoint {d} is mentioned without a relation.",
        ]
    else:
        raise ValueError(f"Unknown mechanism: {mech}")

    # Deterministically permute line order within safe bounds to reduce surface shortcuts.
    # Keep logic line 3 last to preserve readability.
    if mech != "logic":
        rng = random.Random(SEED + idx + MECH_TO_ID[mech] * 10000)
        rng.shuffle(lines)
    ...
    return (
        ...
        f"Record ID: {idx}\n"
        + "\n".join(lines)
        + "\nQuestion: What endpoint is licensed by the record?\n"
        + qform
    )

def build_dataset(n_per_mech: int = 80) -> pd.DataFrame:
    rows = []
    ...
    for mech in MECHANISMS:
        for i in range(n_per_mech):
            ...
            rows.append({
                ...
                # group by entity family across mechanisms; this makes OOF probes harder
                "group_id": f"entity_{i % len(ENTITIES)}",
                "prompt": prompt,
            })
    ...
    # Hard safety: tokenizer requires pure str. If a future edit accidentally
    # creates tuple/list/None prompts, fail early with a useful diagnostic,
    # then coerce to string so the audit can still run.
    bad_mask = ~df["prompt"].map(lambda x: isinstance(x, str))
    if bad_mask.any():
        bad_rows = df.loc[bad_mask, ["row_id", "mechanism", "prompt"]].head(10)
        print("[WARN] Non-string prompts detected in build_dataset; coercing to str.")
        print(bad_rows.to_string(index=False))
        df["prompt"] = df["prompt"].map(lambda x: "" if x is None else ("\n".join(x) if isinstance(x, (list, tuple)) else str(x)))
    else:
...
def extract_hidden_topk_features(
    ...
    for start in range(0, len(df), batch_size):
        ...
        raw_texts = batch["prompt"].tolist()
        texts = []
        for j, x in enumerate(raw_texts):
            if isinstance(x, str):
                texts.append(x)
            elif isinstance(x, (list, tuple)):
                print(f"[WARN] Non-string prompt at global row {start + j}: {type(x)}; joining elements.")
                texts.append("\n".join(map(str, x)))
            elif x is None or (isinstance(x, float) and np.isnan(x)):
                print(f"[WARN] Empty/NaN prompt at global row {start + j}; replacing with empty string.")
                texts.append("")
            else:
                print(f"[WARN] Non-string prompt at global row {start + j}: {type(x)}; coercing to str.")
                texts.append(str(x))
...
def layerwise_oof_mech_probs(hidden: np.ndarray, labels: np.ndarray, groups: np.ndarray = None, n_splits: int = 5):
    """
    hidden: N x L x D
    returns:
        probs: N x L x C
    ...
    if groups is None:
        ...
        split_base = list(cv.split(np.zeros(n), labels))
        cv_mode = "StratifiedKFold"
    else:
        n_group = len(np.unique(groups))
        n_splits_eff = min(n_splits, n_group)
        cv = GroupKFold(n_splits=n_splits_eff)
        split_base = list(cv.split(np.zeros(n), labels, groups))
        cv_mode = "GroupKFold"

    print(f"[probe] oof_cv={cv_mode}, n_splits={len(split_base)}")
...
def build_verdict(layer_metrics: pd.DataFrame, sample_metrics: pd.DataFrame, reg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    first_layer_acc = float(layer_metrics["mech_acc"].iloc[0])
    first_layer_H = float(layer_metrics["H_mech_mean"].iloc[0])
    saturated_probe = (first_layer_acc > 0.95 and first_layer_H < 0.10)

    if saturated_probe:
        verdict = "FAIL_PROBE_SATURATION_LABEL_OR_SURFACE_LEAKAGE"
    elif pass1 and pass2 and pass3 and pass4:
...
def main():
    ...
    parser.add_argument("--n_per_mech", type=int, default=80)
