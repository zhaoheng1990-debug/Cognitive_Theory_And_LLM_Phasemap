# -*- coding: utf-8 -*-
"""
PSA-4C: Window-Aligned Cross-Model Effective Cbit -> Cognitive Quality Audit
============================================================================

Why this version exists
-----------------------
PSA-4B directly reused Qwen-style absolute windows:
...
This worked for Qwen but failed cross-model. In particular, Llama had:

    entry = []

so the stagewise chain degenerated.

PSA-4C fixes the implementation layer:

1. Use model/layer-count aligned relative windows by default.
2. Optionally run multiple window profiles and select by held-out stagewise signal.
3. Keep the same theoretical test:
       stagewise Cbit > direct Cbit
       effective Cbit improves quality prediction
4. No theory change.

Main claim tested
-----------------
Window-aligned stagewise structural Cbit predicts cognitive quality across
models better than direct Cbit.

Run smoke:
    python psa4c_window_aligned_crossmodel_effective_cbit_audit.py --models qwen --n_base 40 --topk 30

Run formal:
    python psa4c_window_aligned_crossmodel_effective_cbit_audit.py --models qwen llama gemma --n_base 120 --topk 50

Try multiple window profiles:
    python psa4c_window_aligned_crossmodel_effective_cbit_audit.py --models qwen llama gemma --n_base 120 --topk 50 --window_mode scan
"""
...
DEFAULT_OUT_DIR = r"C:\Users\ZH\Desktop\AGI\outputs\psa4c_window_aligned_outputs"
SEED = 20260607
    ...
    for i, row in pair_df.iterrows():
        ...
        for cand in candidates:
            ...
            if not math.isfinite(score):
                print(f"[WARN] non-finite candidate score for {cand}; replacing with -1e9")
                score = -1e9
            scores[cand] = score
            token_counts[cand] = n_tok
        ...
        out_rows.append({
            ...
            "candidate_token_counts": json.dumps(token_counts, ensure_ascii=False),
        })
        if (i + 1) % 20 == 0:
...
def make_windows(L, profile="relative_default"):
    """
    Return non-empty model-aligned windows.
    L is number of transformer layers, indexed 0..L-1.
    """
    if profile == "qwen_abs" and L >= 28:
        return {
    ...
    profiles = {
        # Balanced four-stage split, works for 16/26/28 layers.
        "relative_default": {
            "shape": (0.30, 0.65),
            "entry": (0.65, 0.78),
            "commit": (0.78, 0.90),
        ...
        # Earlier commitment, useful for Llama-like short models.
        "relative_early": {
            "shape": (0.25, 0.58),
            "entry": (0.58, 0.72),
            "commit": (0.72, 0.86),
        ...
        # Later commitment, useful for models where final family stabilizes late.
        "relative_late": {
            "shape": (0.35, 0.68),
            "entry": (0.68, 0.80),
            "commit": (0.80, 0.92),
        ...
        # Wide entry window, tests transition-gate hypothesis.
        "relative_wide_entry": {
            "shape": (0.25, 0.60),
            "entry": (0.60, 0.80),
            "commit": (0.80, 0.92),
    ...
    # Ensure all non-empty; if any empty, allocate by minimum discrete split.
...
def evaluate_quality(pair_local, groups, quality_col="answer_margin", correct_col="answer_correct"):
    y_margin = sanitize_array(pair_local[quality_col].values.astype(float), quality_col)
    y_correct = pair_local[correct_col].values.astype(int)
    ...
    for name, cols in feature_sets.items():
        X = pair_local[cols].values
        regression[name + "_to_" + quality_col] = reg_eval(X, y_margin, groups)
        classification[name + "_to_" + correct_col] = clf_eval(X, y_correct, groups)

    corr_stage_margin = safe_corr(pair_local["stagewise_cbit"], y_margin)
    corr_eff_margin = safe_corr(pair_local["effective_cbit_proxy"], y_margin)
    corr_direct_margin = safe_corr(pair_local["cbit_shape_final_direct"], y_margin)

    correct_rate = float(pair_local[correct_col].mean())
    quality_degenerate = correct_rate < 0.05 or correct_rate > 0.95

    stage_r2 = regression["stagewise_cbit_to_" + quality_col]["r2"]
    direct_r2 = regression["direct_cbit_to_" + quality_col]["r2"]
    eff_r2 = regression["effective_cbit_to_" + quality_col]["r2"]

    stage_auc = classification["stagewise_cbit_to_" + correct_col]["auc"]
    direct_auc = classification["direct_cbit_to_" + correct_col]["auc"]
    eff_auc = classification["effective_cbit_to_" + correct_col]["auc"]

    stage_predicts = (
        (corr_stage_margin is not None and abs(corr_stage_margin) > 0.15)
    ...
    effective_improves = (
        ...
        or (corr_eff_margin is not None and corr_stage_margin is not None and abs(corr_eff_margin) > abs(corr_stage_margin) + 0.03)
    )
    stage_beats_direct = (
    ...
    return {
        ...
        "summary_metrics": {
            ...
            "quality_margin_mean": float(pair_local[quality_col].mean()),
            "quality_margin_std": float(pair_local[quality_col].std()),
            "corr_stagewise_cbit_quality_margin": corr_stage_margin,
            "corr_effective_cbit_quality_margin": corr_eff_margin,
            "corr_direct_cbit_quality_margin": corr_direct_margin,
...
def score_window_profile(delta, pair_df, quality, windows, args, profile_name):
    ...
    quality_eval = evaluate_quality(pair_local, groups)

    m = quality_eval["summary_metrics"]
    pf = quality_eval["pass_flags"]

    # Selection score used only for profile selection in scan mode.
    # Prioritize quality prediction and stagewise > direct.
    sel_score = 0.0
    if m["stagewise_correct_auc"] is not None:
        sel_score += max(0.0, m["stagewise_correct_auc"] - 0.5) * 4.0
    if m["effective_correct_auc"] is not None:
        sel_score += max(0.0, m["effective_correct_auc"] - 0.5) * 2.0
    sel_score += max(0.0, m["stagewise_margin_r2"]) * 2.0
    sel_score += max(0.0, m["effective_margin_r2"]) * 2.0
    sel_score += 0.5 if pf["stagewise_beats_direct"] else 0.0
    sel_score += 0.3 if pf["effective_cbit_improves"] else 0.0
...
def run_one_model(model_key, args, prompt_df, pair_df, model_out_dir):
    ...
    quality.to_csv(model_out_dir / f"psa4c_{model_key}_answer_quality.csv", index=False, encoding="utf-8-sig")

    L = delta["hidden_delta"].shape[1]
    if args.window_mode == "scan":
        profiles = ["relative_default", "relative_early", "relative_late", "relative_wide_entry"]
        if model_key == "qwen":
            profiles = ["qwen_abs"] + profiles
    else:
        profiles = ["relative_default"]
        if args.window_mode == "qwen_abs":
            profiles = ["qwen_abs"]
    ...
    for profile in profiles:
        ...
        # Skip invalid profiles if somehow empty.
        if any(len(v) == 0 for v in windows.values()):
    ...
    pair_best.to_csv(model_out_dir / f"psa4c_{model_key}_pairs_with_cbit_quality.csv", index=False, encoding="utf-8-sig")
    ...
    for r in profile_results:
        ...
        profile_table.append({
            ...
            "stagewise_auc": sm["stagewise_correct_auc"],
            "direct_auc": sm["direct_correct_auc"],
            "effective_auc": sm["effective_correct_auc"],
            "stagewise_r2": sm["stagewise_margin_r2"],
            "direct_r2": sm["direct_margin_r2"],
            "effective_r2": sm["effective_margin_r2"],
            "stagewise_predicts": pf["stagewise_cbit_predicts_quality"],
            "stagewise_beats_direct": pf["stagewise_beats_direct"],
            "effective_improves": pf["effective_cbit_improves"],
            "mean_stagewise_cbit": r["mean_stagewise_cbit"],
            "mean_effective_cbit_proxy": r["mean_effective_cbit_proxy"],
            "windows": json.dumps(r["windows"], ensure_ascii=False),
        })
    pd.DataFrame(profile_table).to_csv(model_out_dir / f"psa4c_{model_key}_profile_scan.csv", index=False, encoding="utf-8-sig")

    pf = best["pass_flags"]
    if not pf["quality_non_degenerate"]:
        verdict = "FAIL_DEGENERATE_ANSWER_QUALITY"
    elif pf["stagewise_cbit_predicts_quality"] and pf["effective_cbit_improves"] and pf["stagewise_beats_direct"]:
        verdict = "PASS_PSA4C_WINDOW_ALIGNED_EFFECTIVE_CBIT"
    elif pf["stagewise_cbit_predicts_quality"] and pf["stagewise_beats_direct"]:
        verdict = "PARTIAL_PSA4C_STAGEWISE_CBIT"
    elif pf["stagewise_cbit_predicts_quality"]:
        verdict = "PARTIAL_PSA4C_CBIT_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4C"
    ...
    summary = {
        ...
        "summary_metrics": {
            **best["summary_metrics"],
            "mean_cbit_shape_entry": best["mean_cbit_shape_entry"],
            "mean_cbit_entry_commit": best["mean_cbit_entry_commit"],
            "mean_cbit_commit_final": best["mean_cbit_commit_final"],
            "mean_cbit_shape_final_direct": best["mean_cbit_shape_final_direct"],
            "mean_stagewise_cbit": best["mean_stagewise_cbit"],
            "mean_transition_stability": best["mean_transition_stability"],
            "mean_effective_cbit_proxy": best["mean_effective_cbit_proxy"],
        },
        "regression_results": best["regression_results"],
        "classification_results": best["classification_results"],
        "windows": best["windows"],
        "all_profile_results": profile_table,
        "model_path": MODEL_PATHS[model_key],
    }

    with open(model_out_dir / f"psa4c_{model_key}_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    with open(model_out_dir / f"psa4c_{model_key}_verdict.txt", "w", encoding="utf-8") as f:
...
def aggregate_results(model_summaries):
    ...
    for s in model_summaries:
        ...
        rows.append({
            ...
            "quality_non_degenerate": s["pass_flags"]["quality_non_degenerate"],
            "stagewise_cbit_predicts_quality": s["pass_flags"]["stagewise_cbit_predicts_quality"],
            "effective_cbit_improves": s["pass_flags"]["effective_cbit_improves"],
            "stagewise_beats_direct": s["pass_flags"]["stagewise_beats_direct"],
            "answer_correct_rate": m["answer_correct_rate"],
            "stagewise_margin_r2": m["stagewise_margin_r2"],
            "direct_margin_r2": m["direct_margin_r2"],
            "effective_margin_r2": m["effective_margin_r2"],
            "stagewise_correct_auc": m["stagewise_correct_auc"],
            "direct_correct_auc": m["direct_correct_auc"],
            "effective_correct_auc": m["effective_correct_auc"],
    ...
    n = len(df)
    pass_like = int(df["verdict"].str.contains("PASS|PARTIAL", regex=True).sum())
    stage_beats = int(df["stagewise_beats_direct"].sum())
    eff_improves = int(df["effective_cbit_improves"].sum())
    stage_predicts = int(df["stagewise_cbit_predicts_quality"].sum())

    if pass_like == n and stage_beats >= max(1, n - 1) and stage_predicts >= max(1, n - 1):
        verdict = "PASS_PSA4C_WINDOW_ALIGNED_CROSSMODEL_STAGEWISE_CBIT"
    elif stage_predicts >= max(1, n - 1):
        verdict = "PARTIAL_PSA4C_CROSSMODEL_CBIT_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4C_CROSSMODEL"

    return df, {
        "verdict": verdict,
        "n_models": int(n),
        "n_pass_or_partial": int(pass_like),
        "n_stagewise_predicts": int(stage_predicts),
        "n_stagewise_beats_direct": int(stage_beats),
        "n_effective_improves": int(eff_improves),
        "models": rows,
        "interpretation": (
            "PSA-4C repeats PSA-4B with model/layer-count aligned windows. "
            "If it improves Llama/Gemma, 4B failure was an implementation-window mismatch. "
            "If not, the Cbit->quality mapping may be model-specific or require better quality proxies."
        ),
...
def main():
    ...
    parser.add_argument("--window_mode", default="relative", choices=["relative", "scan", "qwen_abs"])
    ...
    prompt_df.to_csv(out_dir / "psa4c_prompts.csv", index=False, encoding="utf-8-sig")
    pair_df.to_csv(out_dir / "psa4c_pairs.csv", index=False, encoding="utf-8-sig")
    ...
    agg_df.to_csv(out_dir / "psa4c_crossmodel_summary_table.csv", index=False, encoding="utf-8-sig")

    agg["config"] = vars(args)
    with open(out_dir / "psa4c_crossmodel_summary.json", "w", encoding="utf-8") as f:
        json.dump(agg, f, ensure_ascii=False, indent=2)
    with open(out_dir / "psa4c_crossmodel_verdict.txt", "w", encoding="utf-8") as f:
