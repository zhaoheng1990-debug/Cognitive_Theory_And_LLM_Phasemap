...
def process_model(model_key, model_path, df):
    ...
    # Export feature-level raw rows for strict grouped bootstrap.
    # This file is required for Paper4 statistical error bars on non-R vocabulary-neighborhood flow.
    feature_rows = []
    feature_names = [
        "center_shift",
        "jaccard_turnover",
    ...
    base_cols = [c for c in base_cols if c in model_df.columns]
    for k_raw, O_raw in O_by_k.items():
        for l_raw in range(O_raw.shape[1]):
            block = model_df[base_cols].copy()
            block["model_key"] = model_key
            block["k"] = int(k_raw)
            block["transition_layer"] = int(l_raw)
            block["transition"] = f"L{l_raw}->L{l_raw+1}"
            block["decision_start"] = int(dec_start)
            block["DeltaU_decision"] = deltaU.astype(np.float32)
            block["mechanism_class3"] = y3.astype(int)
            F_raw = O_raw[:, l_raw, :]
            for j, fname in enumerate(feature_names):
                block[fname] = F_raw[:, j].astype(np.float32)
            feature_rows.append(block)

    feature_rows_df = pd.concat(feature_rows, ignore_index=True)
    feature_rows_df.to_csv(
        SAVE_DIR / f"{model_key}_cm5c_nonR_feature_rows.csv",
        index=False,
        encoding="utf-8-sig",
    )
