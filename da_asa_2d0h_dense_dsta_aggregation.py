...
# -*- coding: utf-8 -*-
"""
DA-ASA-2D.0h Dense DSTA Aggregation
...
def aggregate_condition_level(df: pd.DataFrame, group: str) -> Optional[pd.DataFrame]:
    ...
    if "window" in df.columns and df["window"].nunique(dropna=True) <= 20:
        pivs = []
        for col in nums:
    ...
    if "topk" in df.columns and df["topk"].nunique(dropna=True) <= 8:
        pivs = []
        for col in nums:
...
def main() -> None:
    ...
    schema = {
        "experiment": "DA-ASA-2D.0h Dense DSTA Aggregation",
