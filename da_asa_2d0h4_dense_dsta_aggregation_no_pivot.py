...
# -*- coding: utf-8 -*-
"""
DA-ASA-2D.0h4 Dense DSTA Aggregation (no-pivot safe)
...
def make_unique_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Make column access safe when CSV/merge created duplicate names.
    ...
    This helper converts column labels to strings and drops duplicate labels,
...
def has_unique_column(df: pd.DataFrame, name: str) -> bool:
    return sum(1 for c in df.columns if c == name) == 1


def safe_rename_to(df: pd.DataFrame, source: str, target: str) -> pd.DataFrame:
    """Rename source to target without leaving duplicate target columns."""
    if source in df.columns and source != target:
        if target in df.columns:
            # Prefer existing target; only fill missing values from source.
...
def aggregate_condition_level(df: pd.DataFrame, group: str) -> Optional[pd.DataFrame]:
    """Aggregate to condition-level dense features.

    Deliberately NO pivot on topk/window. Some DSTA CSVs contain duplicate or
    non-1D topk/window labels after pandas normalization, which can crash
    pivot_table. For 2D.0h the core objective is dense broadcastable DSTA
    features, so condition means are sufficient and safer.
    """
    ...
    if "condition" not in df.columns:
        for c in list(df.columns):
            lc = str(c).lower()
            if lc in ["_condition", "cond"] or lc.endswith("condition"):
                df = safe_rename_to(df, c, "condition")
                break
    df = make_unique_columns(df)
    if "condition" not in df.columns:
        return None

    # If duplicate condition somehow remains, keep only first occurrence.
    if isinstance(df["condition"], pd.DataFrame):
        df["condition"] = df["condition"].iloc[:, 0]
        df = make_unique_columns(df)

    df["condition"] = df["condition"].map(normalize_condition)

    exclude = [
        "condition", "graph_id", "sample_id", "id", "_id", "source_file", "text",
        "top_ids_head", "policy", "applied_actions", "mechanism", "risk_regime",
        "window", "topk", "layer", "_condition", "_mechanism", "_risk", "_surface",
        "_concept"
    ]
    ...
    # Coerce numeric columns explicitly and aggregate means by condition.
    for c in nums:
        df[c] = pd.to_numeric(df[c], errors="coerce")
    out = df.groupby("condition", dropna=False)[nums].mean(numeric_only=True).reset_index()

    # Add lightweight row/count diagnostics; these are dense and safe.
    cnt = df.groupby("condition", dropna=False).size().reset_index(name=f"{group}_n_rows_agg")
    out = out.merge(cnt, on="condition", how="left")
    return out
...
def main() -> None:
    ...
    if args.out_dir:
        ...
        out_dir = base_path.parent.parent / "da_asa_2d0h4_outputs"
    ...
    schema = {
        "experiment": "DA-ASA-2D.0h4 Dense DSTA Aggregation (no-pivot safe)",
