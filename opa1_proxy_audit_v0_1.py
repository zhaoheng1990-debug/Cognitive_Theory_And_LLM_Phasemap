# -*- coding: utf-8 -*-
"""
OPA-1 Ontology Proxy Audit v0.1
...
Inputs:
  CSV with at least:
    prompt_id,prompt
  Recommended:
    condition,family_id,split_group,clean_answer,conflict_answer,phase_label,answer_quality
...
PROMPT_CSV = r"C:\Users\ZH\Desktop\AGI\opa1_prompts.csv"
OUTPUT_DIR = r"C:\Users\ZH\Desktop\AGI\opa1_outputs"

# Change these paths if needed.
MODEL_CONFIGS = [
    ModelCfg(
        key="qwen",
        path=r"D:\model\models--Qwen--Qwen2.5-1.5B-Instruct",
...
RUN_MODEL_KEYS = ["qwen"]  # e.g. ["qwen", "llama", "gemma"]

TOPK = 50
RANDOM_SEED = 42
MAX_PROMPTS = None  # set an integer for smoke test
...
def main():
    ...
    if not os.path.exists(PROMPT_CSV):
        raise FileNotFoundError(
            f"PROMPT_CSV not found: {PROMPT_CSV}\n"
            "Please create a CSV with at least prompt_id,prompt. "
            "A template is provided separately as opa1_prompts_template.csv."
        )
    ...
    df.to_csv(os.path.join(OUTPUT_DIR, "opa1_input_used.csv"), index=False)
