...
EXPERIMENT_ID = "EDA-1C_P4_PSG_ProxyNull_v1_1_batched_hidden_extraction"
...
def extract_all_hidden(model, tokenizer, prompts, layers):
    """
    v1.1 memory patch:
    - Extract hidden states in small batches.
    - Store only final-token hidden states for requested layers on CPU.
    - Immediately delete logits / hidden states after each batch.
    This avoids Gemma OOM caused by full-batch lm_head logits allocation.
    """
    # Determine valid layers with a tiny dry run on one prompt.
    probe = tokenizer(
        [prompts[0]],
        padding=True,
        truncation=True,
    ...
    probe_out = model(**probe, output_hidden_states=True, use_cache=False, return_dict=True)
    n_layers_total = len(probe_out.hidden_states) - 1
    ...
    if not use_layers:
        use_layers = sorted(set([1, n_layers_total // 4, n_layers_total // 2, int(n_layers_total * 0.75), n_layers_total - 1]))

    H_lists = {l: [] for l in use_layers}
    n = len(prompts)

    for start_i in range(0, n, EXTRACT_BATCH_SIZE):
        end_i = min(start_i + EXTRACT_BATCH_SIZE, n)
        ...
        batch_idx = torch.arange(len(batch_prompts), device=DEVICE)

        for l in use_layers:
            Hb = out.hidden_states[l][batch_idx, last_idx, :].detach().float().cpu().numpy().astype(np.float32)
            H_lists[l].append(Hb)

        del out, encoded, last_idx, batch_idx
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        print(f"[INFO] extracted batch {start_i}:{end_i}/{n}")

    H = {l: np.concatenate(H_lists[l], axis=0) for l in use_layers}
    print(f"[INFO] total_layers={n_layers_total}, extracted={len(use_layers)}, batch_size={EXTRACT_BATCH_SIZE}")
    return H, use_layers
