...
# ============================================================
# ASA-8D: Matched Shuffle / Null Validation for Predicted-Mechanism
#        Closed-Loop Control
#
# Motivation:
#   ASA-8C showed:
#
#       predicted-mechanism learned gain policy
#       specificity = 3.0118
#       fixed same_combo specificity = 1.3051
#
#   ASA-8D validates that this is not due to:
#     - policy label artifacts
#     - mechanism classifier leakage
#     - generic sparse intervention
...
#
# Nulls:
#   1. shuffled_mech_policy:
#        train mechanism classifier with shuffled mechanism labels,
#        then train policy on predicted shuffled-mech features.
#
#   2. shuffled_policy_label:
#        keep real mechanism classifier,
#        shuffle best_action labels before training gain policy.
#
#   3. shuffled_both:
#        shuffled mechanism classifier + shuffled policy labels.
#
#   4. random_gain_combo:
#        deterministic random layerwise gains.
#
#   5. fixed same_combo:
#        ASA-7B / ASA-8 baseline.
#
# Default:
#   n_shuffles = 30
#
...
#     asa8d_summary.json
#     asa8d_verdict.csv
#     asa8d_specificity_summary.csv
#     asa8d_shuffle_validation.csv
#     asa8d_steering_results.csv
#     asa8d_real_policy_actions.csv
#     asa8d_control_trace.csv
#     ...
#
# Interpretation:
#   PASS-Strong if:
#     real_predmech_policy > fixed same_combo
#     real_predmech_policy > shuffle_both q95
#     z_vs_shuffle_both > 2
# ============================================================
...
def select_single_token_labels(tokenizer, save_dir):
    ...
    for lab in LABEL_CANDIDATES:
        ...
        if len(ids) == 1:
            chosen.append(lab)
    pd.DataFrame(rows).to_csv(save_dir / "asa8d_label_tokenization_audit.csv", index=False, encoding="utf-8-sig")
    if len(chosen) < 12:
...
def build_dataset(tokenizer, cfg, save_dir):
    ...
    df.to_csv(save_dir / "asa8d_dataset.csv", index=False, encoding="utf-8-sig")
    return df
...
def extract_baseline(model, tokenizer, df, cfg, save_dir):
    ...
    out.to_csv(save_dir / "asa8d_baseline_features.csv", index=False, encoding="utf-8-sig")
...
def make_split(df, cfg, save_dir):
    ...
    split_df.to_csv(save_dir / "asa8d_split.csv", index=False, encoding="utf-8-sig")
...
def fit_stable_operators(base_df, h_by_layer, train_idx, cfg, save_dir):
    ...
    pd.DataFrame(audits).to_csv(save_dir / "asa8d_operator_audit.csv", index=False, encoding="utf-8-sig")
    return operators
...
def fit_precursor_and_probes(base_df, h_by_layer, train_idx, cfg, save_dir):
    ...
    pd.DataFrame(audits).to_csv(save_dir / "asa8d_probe_audit.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{
    ...
    } for l in cfg.precursor_layers]).to_csv(save_dir / "asa8d_direction_audit.csv", index=False, encoding="utf-8-sig")
...
def run_policy_train_grid(model, tokenizer, policy_train_df, cfg, pca_decision, clean_anchor_R,
    ...
    grid_df.to_csv(save_dir / "asa8d_policy_train_grid_results.csv", index=False, encoding="utf-8-sig")
    ...
    label_df.to_csv(save_dir / "asa8d_policy_train_grid_labels.csv", index=False, encoding="utf-8-sig")
...
def main(cfg):
    ...
    with open(save_dir / "asa8d_config.json", "w", encoding="utf-8") as f:
    ...
    real_actions.to_csv(save_dir / "asa8d_real_policy_actions.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(shuffle_audits).to_csv(save_dir / "asa8d_shuffle_policy_audits.csv", index=False, encoding="utf-8-sig")
    with open(save_dir / "asa8d_real_mechanism_audit.json", "w", encoding="utf-8") as f:
        json.dump(real_mech_audit, f, indent=2, ensure_ascii=False)
    with open(save_dir / "asa8d_real_policy_audit.json", "w", encoding="utf-8") as f:
    ...
    results_df.to_csv(save_dir / "asa8d_steering_results.csv", index=False, encoding="utf-8-sig")

    spec_df, merged = summarize_specificity(results_df, base_test)
    spec_df.to_csv(save_dir / "asa8d_specificity_summary.csv", index=False, encoding="utf-8-sig")
    merged.to_csv(save_dir / "asa8d_merged_shift_table.csv", index=False, encoding="utf-8-sig")

    trace_df = pd.concat(traces, ignore_index=True) if len(traces) else pd.DataFrame()
    trace_df.to_csv(save_dir / "asa8d_control_trace.csv", index=False, encoding="utf-8-sig")

    val_df = shuffle_validation(spec_df)
    val_df.to_csv(save_dir / "asa8d_shuffle_validation.csv", index=False, encoding="utf-8-sig")
    ...
    verdict_df = pd.DataFrame([{
        "audit": "ASA-8D Matched Shuffle Validation",
        "verdict": verdict,
    ...
    verdict_df.to_csv(save_dir / "asa8d_verdict.csv", index=False, encoding="utf-8-sig")

    summary = {
        "audit": "ASA-8D Matched Shuffle / Null Validation for Predicted-Mechanism Closed-Loop Control",
        "config": asdict(cfg),
        ...
        "key_outputs": {
            "specificity": "asa8d_specificity_summary.csv",
            "shuffle_validation": "asa8d_shuffle_validation.csv",
            "verdict": "asa8d_verdict.csv",
            "steering_results": "asa8d_steering_results.csv",
            "policy_train_grid_results": "asa8d_policy_train_grid_results.csv",
            "policy_train_grid_labels": "asa8d_policy_train_grid_labels.csv",
            "shuffle_policy_audits": "asa8d_shuffle_policy_audits.csv",
            "real_policy_actions": "asa8d_real_policy_actions.csv",
            "control_trace": "asa8d_control_trace.csv",
        },
        "note": (
            "ASA-8D validates ASA-8C against shuffled mechanism classifier, shuffled policy labels, and shuffled-both nulls. "
    ...
    with open(save_dir / "asa8d_summary.json", "w", encoding="utf-8") as f:
