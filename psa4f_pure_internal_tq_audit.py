# -*- coding: utf-8 -*-
"""
PSA-4F: Pure-Internal TrajectoryQuality Audit
=============================================

Why this version exists
-----------------------
PSA-4E passed cross-model IndependentTQ chain, but its independent TQ included
`tq_answer_rank_quality`, which is answer-adjacent. That makes the strong
TQ -> AnswerQuality result partly expected.

PSA-4F removes all answer-adjacent variables from the TQ target and tests:

    Cbit -> PureInternalTQ -> AnswerQuality

PureInternalTQ proxies
----------------------
These do NOT use answer rank, answer correctness, answer margin, candidate scores,
or any label derived from the correct endpoint.

1. TopK flow smoothness
2. TopK entropy stabilization
3. TopK spread stabilization
4. commit->final TopK Jaccard stability
5. commit->final weighted Jaccard stability
6. within-task final-family consistency
7. optional final-family sharpness

Main tests
----------
A. stagewise Cbit predicts pure-internal TQ.
B. stagewise Cbit beats direct Cbit for pure-internal TQ.
C. pure-internal TQ predicts answer quality.
D. Cbit + pure-internal TQ improves answer-quality prediction.

Run smoke:
    python psa4f_pure_internal_tq_audit.py --models qwen --n_base 40 --topk 30

Formal:
    python psa4f_pure_internal_tq_audit.py --models qwen llama gemma --n_base 120 --topk 50 --window_mode scan
"""
...
DEFAULT_OUT_DIR = r"C:\Users\ZH\Desktop\AGI\outputs\psa4f_pure_internal_tq_outputs"
SEED = 20260607
...
def compute_pure_internal_tq(pair_local, delta, windows):
    """
    Pure internal TQ proxies do NOT use:
    - answer rank
    - answer correctness
    - answer margin
    - candidate scores
    ...
    topk_flow_low_energy = -flow_mean
    ...
    # Within-task final-family consistency is internal w.r.t. endpoint label,
    # but uses task family metadata. Keep it as an internal structure-consistency proxy.
    ...
    out["tq_flow_low_energy"] = topk_flow_low_energy
    out["tq_entropy_stabilization"] = entropy_stabilization
    out["tq_final_entropy_low"] = final_entropy_low
    out["tq_spread_stabilization"] = spread_stabilization
    out["tq_final_spread_low"] = final_spread_low
    ...
    out["tq_pure_internal_composite"] = zsum / len(pure_cols)
    out["tq_pure_internal_high"] = (out["tq_pure_internal_composite"] >= np.median(out["tq_pure_internal_composite"].values)).astype(int)
    return sanitize_dataframe_numeric(out, "pure_internal_tq"), pure_cols
...
def evaluate_pure_tq(pair_local, groups, pure_cols):
    ...
    tq_cols = pure_cols + ["tq_pure_internal_composite"]

    y_tq = pair_local["tq_pure_internal_composite"].values.astype(float)
    y_tq_high = pair_local["tq_pure_internal_high"].values.astype(int)
    ...
    result = {
        ...
        "pure_tq_to_answer_margin": reg_eval(pair_local[tq_cols].values, y_margin, groups),
        "pure_tq_to_answer_correct": clf_eval(pair_local[tq_cols].values, y_correct, groups),
        ...
        "cbit_plus_pure_tq_to_answer_margin": reg_eval(pair_local[cbit_cols + tq_cols].values, y_margin, groups),
        "cbit_plus_pure_tq_to_answer_correct": clf_eval(pair_local[cbit_cols + tq_cols].values, y_correct, groups),
    }

    cbit_r2 = result["cbit_to_pure_tq"]["regression"]["r2"]
    direct_r2 = result["direct_to_pure_tq"]["regression"]["r2"]
    cbit_auc = result["cbit_to_pure_tq"]["classification"]["auc"]
    direct_auc = result["direct_to_pure_tq"]["classification"]["auc"]
    tq_ans_r2 = result["pure_tq_to_answer_margin"]["r2"]
    tq_ans_auc = result["pure_tq_to_answer_correct"]["auc"]
    cbit_ans_r2 = result["cbit_to_answer_margin"]["r2"]
    cbit_ans_auc = result["cbit_to_answer_correct"]["auc"]
    plus_ans_r2 = result["cbit_plus_pure_tq_to_answer_margin"]["r2"]
    plus_ans_auc = result["cbit_plus_pure_tq_to_answer_correct"]["auc"]

    flags = {
        "cbit_predicts_pure_tq": bool((cbit_r2 > 0.05) or (cbit_auc is not None and cbit_auc > 0.62)),
        "stagewise_beats_direct_for_pure_tq": bool((cbit_r2 > direct_r2 + 0.03) or (cbit_auc is not None and direct_auc is not None and cbit_auc > direct_auc + 0.03)),
        "pure_tq_predicts_answer": bool((tq_ans_r2 > 0.03) or (tq_ans_auc is not None and tq_ans_auc > 0.58)),
        "pure_tq_improves_answer_model": bool((plus_ans_r2 > cbit_ans_r2 + 0.02) or (plus_ans_auc is not None and cbit_ans_auc is not None and plus_ans_auc > cbit_ans_auc + 0.03)),
    }

    summary = {
        "cbit_to_pure_tq_r2": cbit_r2,
        "cbit_to_pure_tq_auc": cbit_auc,
        "direct_to_pure_tq_r2": direct_r2,
        "direct_to_pure_tq_auc": direct_auc,
        "pure_tq_to_answer_margin_r2": tq_ans_r2,
        "pure_tq_to_answer_correct_auc": tq_ans_auc,
        ...
        "mean_tq_pure_internal_composite": float(pair_local["tq_pure_internal_composite"].mean()),
        "std_tq_pure_internal_composite": float(pair_local["tq_pure_internal_composite"].std()),
    }
...
def score_window_profile(delta, pair_df, quality, windows, args, profile_name):
    ...
    pair_local, pure_cols = compute_pure_internal_tq(pair_local, delta, windows)
    pair_local = sanitize_dataframe_numeric(pair_local, f"{profile_name}.pair_local_final")

    evals, flags, summary = evaluate_pure_tq(pair_local, groups, pure_cols)

    sel_score = 0.0
    sel_score += max(0.0, summary["cbit_to_pure_tq_r2"]) * 3.0
    if summary["cbit_to_pure_tq_auc"] is not None:
        sel_score += max(0.0, summary["cbit_to_pure_tq_auc"] - 0.5) * 3.0
    if summary["pure_tq_to_answer_correct_auc"] is not None:
        sel_score += max(0.0, summary["pure_tq_to_answer_correct_auc"] - 0.5) * 2.0
    sel_score += max(0.0, summary["pure_tq_to_answer_margin_r2"]) * 2.0
    sel_score += 0.5 if flags["stagewise_beats_direct_for_pure_tq"] else 0.0
...
def run_one_model(model_key, args, prompt_df, pair_df, model_out_dir):
    ...
    quality.to_csv(model_out_dir / f"psa4f_{model_key}_answer_quality.csv", index=False, encoding="utf-8-sig")
    ...
    pair_best.to_csv(model_out_dir / f"psa4f_{model_key}_pairs_with_pure_internal_tq.csv", index=False, encoding="utf-8-sig")
    ...
    for r in profile_results:
        ...
        profile_table.append({
            ...
            "cbit_to_pure_tq_r2": sm["cbit_to_pure_tq_r2"],
            "cbit_to_pure_tq_auc": sm["cbit_to_pure_tq_auc"],
            "direct_to_pure_tq_r2": sm["direct_to_pure_tq_r2"],
            "direct_to_pure_tq_auc": sm["direct_to_pure_tq_auc"],
            "pure_tq_to_answer_correct_auc": sm["pure_tq_to_answer_correct_auc"],
            "pure_tq_to_answer_margin_r2": sm["pure_tq_to_answer_margin_r2"],
            ...
            "cbit_predicts_pure_tq": pf["cbit_predicts_pure_tq"],
            "stagewise_beats_direct_pure_tq": pf["stagewise_beats_direct_for_pure_tq"],
            "pure_tq_predicts_answer": pf["pure_tq_predicts_answer"],
            "pure_tq_improves_answer_model": pf["pure_tq_improves_answer_model"],
    ...
    pd.DataFrame(profile_table).to_csv(model_out_dir / f"psa4f_{model_key}_profile_scan.csv", index=False, encoding="utf-8-sig")

    pf = best["pass_flags"]
    if pf["cbit_predicts_pure_tq"] and pf["stagewise_beats_direct_for_pure_tq"] and pf["pure_tq_predicts_answer"]:
        verdict = "PASS_PSA4F_CBIT_TO_PURE_TQ_TO_ANSWER"
    elif pf["cbit_predicts_pure_tq"] and pf["stagewise_beats_direct_for_pure_tq"]:
        verdict = "PARTIAL_PSA4F_CBIT_TO_PURE_TQ"
    elif pf["cbit_predicts_pure_tq"]:
        verdict = "PARTIAL_PSA4F_CBIT_PURE_TQ_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4F"
    ...
    summary = {
        ...
        "pure_internal_tq_columns": best["pure_internal_cols"],
        "windows": best["windows"],
        "all_profile_results": profile_table,
        "model_path": MODEL_PATHS[model_key],
    }

    with open(model_out_dir / f"psa4f_{model_key}_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    with open(model_out_dir / f"psa4f_{model_key}_verdict.txt", "w", encoding="utf-8") as f:
...
def aggregate_results(model_summaries):
    ...
    for s in model_summaries:
        ...
        rows.append({
            ...
            "cbit_predicts_pure_tq": s["pass_flags"]["cbit_predicts_pure_tq"],
            "stagewise_beats_direct_pure_tq": s["pass_flags"]["stagewise_beats_direct_for_pure_tq"],
            "pure_tq_predicts_answer": s["pass_flags"]["pure_tq_predicts_answer"],
            "pure_tq_improves_answer_model": s["pass_flags"]["pure_tq_improves_answer_model"],
            "cbit_to_pure_tq_r2": m["cbit_to_pure_tq_r2"],
            "cbit_to_pure_tq_auc": m["cbit_to_pure_tq_auc"],
            "direct_to_pure_tq_r2": m["direct_to_pure_tq_r2"],
            "direct_to_pure_tq_auc": m["direct_to_pure_tq_auc"],
            "pure_tq_to_answer_correct_auc": m["pure_tq_to_answer_correct_auc"],
            "pure_tq_to_answer_margin_r2": m["pure_tq_to_answer_margin_r2"],
    ...
    n = len(df)
    n_cbit_tq = int(df["cbit_predicts_pure_tq"].sum())
    n_stage_beats = int(df["stagewise_beats_direct_pure_tq"].sum())
    n_tq_answer = int(df["pure_tq_predicts_answer"].sum())
    ...
    if n_cbit_tq >= max(1, n - 1) and n_stage_beats >= max(1, n - 1) and n_tq_answer >= max(1, n - 1):
        verdict = "PASS_PSA4F_CROSSMODEL_PURE_INTERNAL_TQ_CHAIN"
    elif n_cbit_tq >= max(1, n - 1) and n_stage_beats >= max(1, n - 1):
        verdict = "PARTIAL_PSA4F_CROSSMODEL_CBIT_TO_PURE_TQ"
    elif n_cbit_tq >= max(1, n - 1):
        verdict = "PARTIAL_PSA4F_CROSSMODEL_CBIT_PURE_TQ_SIGNAL"
    else:
        verdict = "FAIL_OR_CAVEAT_PSA4F_CROSSMODEL"

    return df, {
        "verdict": verdict,
        "n_models": int(n),
        "n_pass_or_partial": n_pass_or_partial,
        "n_cbit_predicts_pure_tq": n_cbit_tq,
        "n_stagewise_beats_direct_for_pure_tq": n_stage_beats,
        "n_pure_tq_predicts_answer": n_tq_answer,
        "models": rows,
        "interpretation": (
            "PSA-4F removes answer-adjacent features from TQ. It tests whether stagewise Cbit predicts "
            "pure internal trajectory quality and whether pure internal quality still predicts answer quality."
        ),
        "terminology": "Cbit / pure-internal trajectory quality / answer quality / stagewise transition; no basin terminology",
    }
...
def main():
    ...
    prompt_df.to_csv(out_dir / "psa4f_prompts.csv", index=False, encoding="utf-8-sig")
    pair_df.to_csv(out_dir / "psa4f_pairs.csv", index=False, encoding="utf-8-sig")
    ...
    agg_df.to_csv(out_dir / "psa4f_crossmodel_summary_table.csv", index=False, encoding="utf-8-sig")

    agg["config"] = vars(args)
    with open(out_dir / "psa4f_crossmodel_summary.json", "w", encoding="utf-8") as f:
        json.dump(agg, f, ensure_ascii=False, indent=2)
    with open(out_dir / "psa4f_crossmodel_verdict.txt", "w", encoding="utf-8") as f:
