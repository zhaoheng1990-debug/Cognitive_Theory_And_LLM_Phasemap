# ============================================================
# ASA-1C: Shuffle Specificity Audit for Q->S Residual Prototype Steering
# ------------------------------------------------------------
# Goal:
...
def _remove_subspace(vec: np.ndarray, basis: np.ndarray) -> np.ndarray:
    """Remove the shared PCA subspace from a direction and renormalize."""
...
def _raw_direction_from_proto(pdict, direction_name: str, layer: int) -> np.ndarray:
    if direction_name == "Q_to_S":
        d = pdict["stable"][layer] - pdict["closure"][layer]
    elif direction_name == "K_to_S":
        d = pdict["stable"][layer] - pdict["competition"][layer]
    elif direction_name == "S_to_Q":
        d = pdict["closure"][layer] - pdict["stable"][layer]
    else:
...
def build_prototypes(df: pd.DataFrame, split: pd.DataFrame, centers: Dict[int, np.ndarray], cfg: Config, save_dir: Path):
    """
    ASA-1B prototype builder.

    Compared with ASA-1, this builds:
      1. raw mechanism prototypes P_S/P_K/P_Q;
      2. shared PCA subspaces from all train TopK centers;
      3. residualized directions with the shared subspace removed;
      4. a shuffle distribution of residual directions for significance testing.
    """
    train_mask = split["split"].values == "train"
    mechanisms = ["stable", "competition", "closure"]
    protos = {m: {} for m in mechanisms}
    shared_basis = {}
    shuffle_dirs = []
    rows = []

    rng = np.random.default_rng(cfg.seed + 2026)
    mech_labels_all = df["mechanism"].values

    # Raw prototypes and shared basis per layer.
    for l in cfg.proto_layers:
        C = centers[l].astype(np.float32)
        C_train = C[train_mask]

        # PCA components define high-variance shared TopK-flow subspace.
        n_comp = min(int(cfg.shared_pca_dim), C_train.shape[0] - 1, C_train.shape[1])
        if n_comp > 0:
            pca = PCA(n_components=n_comp)
            pca.fit(C_train)
            B = pca.components_.astype(np.float32)
        else:
            B = np.zeros((0, C_train.shape[1]), dtype=np.float32)
        shared_basis[l] = B

        for mech in mechanisms:
            mask = train_mask & (mech_labels_all == mech)
    ...
    # Shuffle residual direction distribution. Labels are permuted within train rows.
    train_indices = np.where(train_mask)[0]
    train_labels = mech_labels_all[train_indices].copy()

    for si in range(int(cfg.n_shuffles)):
        perm_labels = rng.permutation(train_labels)
        sp = {m: {} for m in mechanisms}
        for l in cfg.proto_layers:
            C = centers[l].astype(np.float32)
            for mech in mechanisms:
                idx = train_indices[perm_labels == mech]
                if len(idx) == 0:
                    # Fallback: should not happen with balanced labels.
                    idx = train_indices
                proto = C[idx].mean(axis=0)
                proto = proto / (np.linalg.norm(proto) + 1e-8)
                sp[mech][l] = proto.astype(np.float32)
        # Store residualized directions for all supported direction names.
        sd = {"Q_to_S": {}, "K_to_S": {}, "S_to_Q": {}}
        for dname in sd:
            for l in cfg.proto_layers:
                raw = _raw_direction_from_proto(sp, dname, l)
                sd[dname][l] = _remove_subspace(raw, shared_basis[l])
        shuffle_dirs.append(sd)

    protos["__shared_basis__"] = shared_basis
    protos["__shuffle_dirs__"] = shuffle_dirs

    proto_df = pd.DataFrame(rows)
    proto_df.to_csv(save_dir / "asa1b_prototype_audit.csv", index=False, encoding="utf-8-sig")
    proto_df.to_csv(save_dir / "asa1_prototype_audit.csv", index=False, encoding="utf-8-sig")

    np.savez_compressed(
        save_dir / "asa1b_prototypes_and_shared_basis.npz",
        **{f"{m}_L{l}": protos[m][l] for m in mechanisms for l in cfg.proto_layers},
        **{f"shared_basis_L{l}": shared_basis[l] for l in cfg.proto_layers},
    )
    return protos


def build_direction(protos, direction_name: str, layer: int, variant: str = "raw", shuffle_idx: Optional[int] = None) -> np.ndarray:
    if shuffle_idx is not None:
        return protos["__shuffle_dirs__"][int(shuffle_idx)][direction_name][layer].astype(np.float32)

    raw = _raw_direction_from_proto(protos, direction_name, layer)
    if variant == "raw":
        return raw
    if variant == "residual":
        return _remove_subspace(raw, protos["__shared_basis__"][layer])
    raise ValueError(f"Unknown direction variant: {variant}")


def audit_directions(protos, cfg: Config, save_dir: Path):
    rows = []
    dirs = ["Q_to_S"]  # ASA-1C focuses on closure -> stable repair and shuffle-specificity statistics.
    for dname in dirs:
        for l in cfg.proto_layers:
            raw = build_direction(protos, dname, l, variant="raw")
            res = build_direction(protos, dname, l, variant="residual")
            cos_raw_res = float(np.dot(raw, res) / ((np.linalg.norm(raw) * np.linalg.norm(res)) + 1e-8))
            rows.append({
                "direction": dname,
                "layer": l,
                "raw_norm": float(np.linalg.norm(raw)),
                "residual_norm": float(np.linalg.norm(res)),
                "cos_raw_residual": cos_raw_res,
                "shared_pca_dim": int(protos["__shared_basis__"][l].shape[0]),
            })
    out = pd.DataFrame(rows)
    out.to_csv(save_dir / "asa1b_direction_audit.csv", index=False, encoding="utf-8-sig")
...
def make_batch_dirs(direction_name: str, control: str, batch_rows: pd.DataFrame, layers: List[int], protos,
    ...
    """
    Controls in ASA-1B:
      prototype_raw        : raw P_S-P_Q
      prototype_residual   : shared-subspace-removed P_S-P_Q
      orthogonal_raw       : raw direction with answer direction removed per sample
      orthogonal_residual  : residual direction with answer direction removed per sample
      random               : random unit direction
      answer               : answer-token positive control
      shuffle_residual_### : label-shuffled residual prototype direction
    """
    bsz = len(batch_rows)
    dim = W_np.shape[1]
    out = {}
    ...
    shuffle_idx = None
    if control.startswith("shuffle_residual_"):
        shuffle_idx = int(control.split("_")[-1])

    for l in layers:
        if control == "prototype_raw":
            d = build_direction(protos, direction_name, l, variant="raw")
        ...
        elif control == "prototype_residual":
            d = build_direction(protos, direction_name, l, variant="residual")
            mat = np.repeat(d[None, :], bsz, axis=0)
            dirs = torch.tensor(mat, dtype=torch.float32, device=device)
            dirs = dirs / (torch.norm(dirs, dim=1, keepdim=True) + 1e-8)
        elif control == "orthogonal_raw":
            d = build_direction(protos, direction_name, l, variant="raw")
            mat = np.repeat(d[None, :], bsz, axis=0)
            dirs = torch.tensor(mat, dtype=torch.float32, device=device)
            dirs = dirs / (torch.norm(dirs, dim=1, keepdim=True) + 1e-8)
            dirs = project_orthogonal(dirs, ans)
        elif control == "orthogonal_residual":
            d = build_direction(protos, direction_name, l, variant="residual")
            mat = np.repeat(d[None, :], bsz, axis=0)
            dirs = torch.tensor(mat, dtype=torch.float32, device=device)
            dirs = dirs / (torch.norm(dirs, dim=1, keepdim=True) + 1e-8)
            ...
            d = build_direction(protos, direction_name, l, shuffle_idx=shuffle_idx)
            mat = np.repeat(d[None, :], bsz, axis=0)
            dirs = torch.tensor(mat, dtype=torch.float32, device=device)
            dirs = dirs / (torch.norm(dirs, dim=1, keepdim=True) + 1e-8)
        else:
...
def summarize_results(results: pd.DataFrame, base_lookup: pd.DataFrame, save_dir: Path):
    ...
    for (direction, window, alpha), sub in merged.groupby(["direction", "window", "alpha"]):
        if alpha == 0:
            continue
        proto = sub[sub["control"] == "prototype_residual"]
        if len(proto) == 0:
            continue
        for ctrl in ["prototype_raw", "random", "orthogonal_residual", "answer"] + [c for c in sorted(sub["control"].unique()) if str(c).startswith("shuffle_residual_")]:
            ...
            verdict_rows.append({
                ...
                "proto_control": "prototype_residual",
...
def main(cfg: Config):
    ...
    directions = ["Q_to_S"]
    base_controls = ["prototype_raw", "prototype_residual", "orthogonal_raw", "orthogonal_residual", "random", "answer"]
    shuffle_controls = [f"shuffle_residual_{i:03d}" for i in range(int(cfg.n_shuffles))]
        ...
        for direction in directions:
            for control in base_controls + shuffle_controls:
                for alpha in cfg.alphas:
                    # Shuffle distribution is expensive; run it only for selected nonzero alpha values.
                    if control.startswith("shuffle_residual_") and float(alpha) not in set(map(float, cfg.shuffle_alphas)):
                        continue
                    if control.startswith("shuffle_residual_") and float(alpha) == 0.0:
                        continue
    ...
    # ASA-1B shuffle-distribution significance summary.
    b = merged.copy()
    proto = b[b["control"] == "prototype_residual"].groupby(["direction", "window", "alpha"]).agg(
        proto_abs_shift=("DeltaU_shift", lambda x: float(np.mean(np.abs(x)))),
        proto_mean_shift=("DeltaU_shift", "mean"),
        proto_target_hit=("target_hit", "mean"),
    ).reset_index()
    sh = b[b["control"].astype(str).str.startswith("shuffle_residual_")].groupby(["direction", "window", "alpha", "control"]).agg(
        shuffle_abs_shift=("DeltaU_shift", lambda x: float(np.mean(np.abs(x)))),
        shuffle_mean_shift=("DeltaU_shift", "mean"),
        shuffle_target_hit=("target_hit", "mean"),
    ).reset_index()
    shsum = sh.groupby(["direction", "window", "alpha"]).agg(
        shuffle_n=("control", "count"),
        shuffle_abs_mean=("shuffle_abs_shift", "mean"),
        shuffle_abs_std=("shuffle_abs_shift", "std"),
        shuffle_mean_shift_mean=("shuffle_mean_shift", "mean"),
        shuffle_target_hit_mean=("shuffle_target_hit", "mean"),
    ).reset_index()
    sig = proto.merge(shsum, on=["direction", "window", "alpha"], how="left")
    sig["proto_minus_shuffle_abs"] = sig["proto_abs_shift"] - sig["shuffle_abs_mean"]
    sig["shuffle_z_abs"] = sig["proto_minus_shuffle_abs"] / (sig["shuffle_abs_std"].fillna(0) + 1e-8)
    sig.to_csv(save_dir / "asa1b_shuffle_significance.csv", index=False, encoding="utf-8-sig")



    # ASA-1C shuffle-distribution SPECIFICITY significance summary.
    # This is the main ASA-1C audit. ASA-1B only tested global abs shift against shuffle;
    # ASA-1C tests whether target-specific shift exceeds shuffle specificity.
    spec_all = spec.copy()
    proto_spec = spec_all[spec_all["control"] == "prototype_residual"][[
    ...
    orth_spec = spec_all[spec_all["control"] == "orthogonal_residual"][[
    ...
    rand_spec = spec_all[spec_all["control"] == "random"][[
        "direction", "window", "alpha", "specificity_abs_shift", "target_abs_shift", "nontarget_abs_shift"
    ]].rename(columns={
    ...
    sh_spec_each = spec_all[spec_all["control"].astype(str).str.startswith("shuffle_residual_")][[
    ...
    sh_spec_each.to_csv(save_dir / "asa1c_shuffle_specificity_each.csv", index=False, encoding="utf-8-sig")
    ...
    spec_sig["pass_specificity_z2"] = spec_sig["shuffle_z_specificity"] > 2.0
    spec_sig["pass_specificity_positive"] = spec_sig["proto_specificity"] > 0
    spec_sig["pass_proto_gt_random"] = spec_sig["proto_specificity"] > spec_sig["random_specificity"]
    spec_sig.to_csv(save_dir / "asa1c_shuffle_specificity_significance.csv", index=False, encoding="utf-8-sig")

    # Compact ASA-1C verdict at the max alpha run.
    verdict_c_rows = []
    if len(spec_sig) > 0:
        max_alpha = float(np.nanmax(spec_sig["alpha"].values))
        for _, row in spec_sig[spec_sig["alpha"] == max_alpha].iterrows():
            verdict_c_rows.append({
    ...
    verdict_c = pd.DataFrame(verdict_c_rows)
    verdict_c.to_csv(save_dir / "asa1c_verdict.csv", index=False, encoding="utf-8-sig")

    # Summary JSON.
    summary = {
        "audit": "ASA-1C Shuffle Specificity Audit Q_to_S",
        "config": asdict(cfg),
        ...
        "key_outputs": {
            ...
            "shuffle_significance": "asa1b_shuffle_significance.csv",
            "shuffle_specificity_significance": "asa1c_shuffle_specificity_significance.csv",
            "shuffle_specificity_each": "asa1c_shuffle_specificity_each.csv",
            "asa1c_verdict": "asa1c_verdict.csv",
        },
        "note": (
            "ASA-1C uses residualized TopK-center prototype directions and evaluates whether closure-specific steering exceeds a 50-shuffle specificity distribution. Answer-token logits/ranks/margins are not used "
