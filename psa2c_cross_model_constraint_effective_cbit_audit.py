# -*- coding: utf-8 -*-
r"""
PSA-2C: Unified Full-Coverage ConstraintField -> Effective Cbit Audit
...
PSA-2C removes this caveat by extracting both feature families from the same
...
Default models
--------------
Qwen / Llama / Gemma cross-model run:
...
Outputs
-------
C:\Users\ZH\Desktop\AGI\outputs\psa2c_outputs\
  psa2c_<model>_features.csv
  psa2c_<model>_model_summary.csv
  psa2c_<model>_group_summary.csv
  psa2c_<model>_verdict.json
  psa2c_cross_model_summary.csv
  psa2c_verdict.json

Verdict logic
-------------
PASS_PSA2C_CONSTRAINT_TO_EFFECTIVE_CBIT:
  CFT_NoAnswer predicts Cbit_eff_basin and correct samples have higher Cbit_eff.

PARTIAL_PSA2C_RAW_CBIT_ONLY:
...
OUT_DIR = Path(r"C:\Users\ZH\Desktop\AGI\outputs\psa2c_outputs")
...
def verdict_for_model(model_key, df, model_summary, gsummary):
    ...
    if np.isfinite(cft_eff_corr) and cft_eff_corr > 0.30 and np.isfinite(eff_gap) and eff_gap > 0:
        verdict = "PASS_PSA2C_CONSTRAINT_TO_EFFECTIVE_CBIT"
        ...
        verdict = "PARTIAL_PSA2C_RAW_CBIT_ONLY"
        ...
        verdict = "PARTIAL_PSA2C_EFFECTIVE_CBIT_SEPARATES"
...
def run_one_model(model_key):
    ...
    dataset.to_csv(model_out / f"psa2c_{model_key}_dataset.csv", index=False, encoding="utf-8-sig")
    ...
    try:
        ...
        feat_df.to_csv(model_out / f"psa2c_{model_key}_features.csv", index=False, encoding="utf-8-sig")
        err_df.to_csv(model_out / f"psa2c_{model_key}_errors.csv", index=False, encoding="utf-8-sig")
        ...
        msummary.to_csv(model_out / f"psa2c_{model_key}_model_summary.csv", index=False, encoding="utf-8-sig")
        gsummary.to_csv(model_out / f"psa2c_{model_key}_group_summary.csv", index=False, encoding="utf-8-sig")
        ...
        verdict.update({
            ...
            "outputs": {
                "features": str(model_out / f"psa2c_{model_key}_features.csv"),
                "model_summary": str(model_out / f"psa2c_{model_key}_model_summary.csv"),
                "group_summary": str(model_out / f"psa2c_{model_key}_group_summary.csv"),
            },
            "top_rows": msummary.sort_values(["target", "corr", "r2"], ascending=[True, False, False]).head(20).to_dict(orient="records"),
        })

        with open(model_out / f"psa2c_{model_key}_verdict.json", "w", encoding="utf-8") as f:
...
def main():
    ...
    for model_key in MODELS_TO_RUN:
        print("=" * 80)
        print(f"[PSA-2C] Running model: {model_key}")
    ...
    cross.to_csv(OUT_DIR / "psa2c_cross_model_summary.csv", index=False, encoding="utf-8-sig")
    ...
    if len(done) == 0:
        global_verdict = "FAIL_PSA2C_NO_MODELS"
    elif pass_count == len(done):
        global_verdict = "PASS_PSA2C"
    elif pass_count + partial_count == len(done):
        global_verdict = "PARTIAL_PSA2C"
    elif pass_count >= 1:
        global_verdict = "MIXED_PSA2B"
    else:
        global_verdict = "FAIL_PSA2C"
    ...
    global_obj = {
        ...
        "models": all_infos,
        "outputs": {
            "cross_summary": str(OUT_DIR / "psa2c_cross_model_summary.csv"),
        },
    }

    with open(OUT_DIR / "psa2c_verdict.json", "w", encoding="utf-8") as f:
