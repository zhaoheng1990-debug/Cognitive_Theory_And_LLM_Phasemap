# ============================================================
# ASA-1B: Residual Prototype Steering for Q->S
# ------------------------------------------------------------
# Goal:
...
class Config:
    ...
    # ASA-1B focuses on the Q->S closure-repair window suggested by ASA-1-Qwen.
    ...
    # ASA-1B focuses on closure -> stable repair.
...
def audit_directions(protos, cfg: Config, save_dir: Path):
    rows = []
    dirs = ["Q_to_S"]  # ASA-1B focuses on closure -> stable repair.
    for dname in dirs:
        for l in cfg.proto_layers:
...
def main(cfg: Config):
    ...
    # Summary JSON.
    summary = {
        "audit": "ASA-1B Residual Prototype Steering Q_to_S",
        "config": asdict(cfg),
        ...
        "note": (
            "ASA-1B uses residualized TopK-center prototype directions and a 50-shuffle distribution. Answer-token logits/ranks/margins are not used "
